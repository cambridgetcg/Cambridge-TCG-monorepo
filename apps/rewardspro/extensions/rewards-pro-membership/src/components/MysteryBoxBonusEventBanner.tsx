...
export interface MysteryBoxBonusEventInfo {
  id: string;
  name: string;
...
interface MysteryBoxBonusEventBannerProps {
  event: MysteryBoxBonusEventInfo;
...
function getBonusDescription(event: MysteryBoxBonusEventInfo): string {
  const parts: string[] = [];

  if (event.bonusMultiplier > 1) {
    parts.push(`${event.bonusMultiplier}x rewards`);
  }
...
export function MysteryBoxBonusEventBanner({
  event,
  translate,
}: MysteryBoxBonusEventBannerProps) {
      ...
      <BlockStack spacing="tight">
        ...
        {isUrgent && (
          <Text size="small" emphasis="bold">
            {translate('mysteryBoxes.bonusEndingSoon')}
          </Text>
        )}
      </BlockStack>
    </View>
...
interface MysteryBoxBonusEventListProps {
  events: MysteryBoxBonusEventInfo[];
  translate: (key: string, options?: Record<string, string>) => string;
}

export function MysteryBoxBonusEventList({
  events,
  translate,
}: MysteryBoxBonusEventListProps) {
