...
/**
 * Apply StoreCreditSyncJob Migration to Production Database via AWS Data API
 *
 * This script creates the StoreCreditSyncJob table:
 * 1. Verifies SyncJobStatus enum exists (should already exist from CustomerSyncJob)
 * 2. Creates StoreCreditSyncJob table with all columns
 * 3. Creates necessary indexes for performance
 *
 * Run with: node scripts/apply-store-credit-sync-job-migration.mjs
 */
...
async function applyStoreCreditSyncJobMigration() {
  ...
  console.log("StoreCreditSyncJob Migration Script");
  console.log("=".repeat(60));
  console.log("");
  ...
  try {
    ...
    console.log("✅ StoreCreditSyncJob migration completed successfully!\n");
...
async function executeMigrationSteps(client, resourceArn, secretArn, database, transactionId) {
  ...
  // ============================================================================
  // Step 1: Check if SyncJobStatus enum exists (should exist from CustomerSyncJob)
  // ============================================================================
  ...
  // ============================================================================
  // Step 3: Check if StoreCreditSyncJob table exists
  // ============================================================================
  console.log("\nStep 3: Checking for existing StoreCreditSyncJob table...");
  ...
  const tableCheckResult = await client.send(new ExecuteStatementCommand({
    ...
    sql: `SELECT table_name FROM information_schema.tables
          WHERE table_name = 'StoreCreditSyncJob' AND table_schema = 'public'`,
    transactionId,
  }));

  const tableExists = (tableCheckResult.records || []).length > 0;
  console.log(`  StoreCreditSyncJob table exists: ${tableExists}`);

  // ============================================================================
  // Step 4: Create StoreCreditSyncJob table if it doesn't exist
  // ============================================================================
  if (!tableExists) {
    console.log("\nStep 4: Creating StoreCreditSyncJob table...");
    ...
    await client.send(new ExecuteStatementCommand({
      ...
      database,
      sql: `
        CREATE TABLE "StoreCreditSyncJob" (
          "id" TEXT NOT NULL,
          "shop" TEXT NOT NULL,
          ...
          -- Financial summary
          "totalImported" DECIMAL(10, 2) NOT NULL DEFAULT 0,
          ...
          CONSTRAINT "StoreCreditSyncJob_pkey" PRIMARY KEY ("id")
        )
      `,
      transactionId,
    }));
    console.log("  ✓ StoreCreditSyncJob table created");

  } else {
    console.log("\nStep 4: StoreCreditSyncJob table already exists (skipping table creation)");
  }
  ...
  const indexes = [
    { name: 'StoreCreditSyncJob_shop_status_idx', columns: '"shop", "status"' },
    { name: 'StoreCreditSyncJob_shop_createdAt_idx', columns: '"shop", "createdAt" DESC' },
  ];
  ...
  for (const index of indexes) {
    try {
      await client.send(new ExecuteStatementCommand({
        ...
        secretArn,
        database,
        sql: `CREATE INDEX IF NOT EXISTS "${index.name}" ON "StoreCreditSyncJob"(${index.columns})`,
        transactionId,
      }));
  ...
  const migrationName = `20251218_add_store_credit_sync_job`;
  ...
  try {
    ...
    if ((migrationCheck.records || []).length === 0) {
      await client.send(new ExecuteStatementCommand({
        resourceArn,
        secretArn,
        database,
        ...
        parameters: [
          ...
          { name: "checksum", value: { stringValue: "store_credit_sync_job_v1" }},
          { name: "name", value: { stringValue: migrationName }},
          { name: "logs", value: { stringValue: "Applied StoreCreditSyncJob migration via Data API" }},
        ],
        transactionId,
      }));
      console.log(`  ✓ Migration recorded: ${migrationName}`);
    } else {
      console.log(`  ⚠️  Migration already recorded: ${migrationName}`);
    }
  } catch (error) {
...
async function verifyChanges(client, resourceArn, secretArn, database) {
  ...
  // Check StoreCreditSyncJob table structure
  console.log("📋 StoreCreditSyncJob Table Columns:");
  try {
    const tableResult = await client.send(new ExecuteStatementCommand({
      ...
      sql: `SELECT column_name, data_type, is_nullable, column_default
            ...
            WHERE table_name = 'StoreCreditSyncJob'
  ...
  try {
    const indexResult = await client.send(new ExecuteStatementCommand({
      ...
      sql: `SELECT indexname FROM pg_indexes
            WHERE tablename = 'StoreCreditSyncJob'
            ORDER BY indexname`,
    }));
  ...
  // Check row count
  console.log("\n📋 StoreCreditSyncJob Row Count:");
  try {
    const countResult = await client.send(new ExecuteStatementCommand({
      resourceArn,
      secretArn,
      database,
      sql: `SELECT COUNT(*) as count FROM "StoreCreditSyncJob"`,
    }));
...
// Run the migration
applyStoreCreditSyncJobMigration().catch(error => {
