/**
 * Top-level facade — walks `app/` and validates against the canonical
 * raffle contract. Reuses the same I/O shape as `ledger-validator`.
 */
...
function walk(root: string, dir: string, out: ScannedFile[]): void {
  for (const entry of fs.readdirSync(dir)) {
    ...
    out.push({
      path: path
        .relative(path.resolve(__dirname, "../.."), full)
        .replace(/\\/g, "/"),
...
export function validateRaffleContract(): Report {
  return validate(loadAppSources(), raffleContract);
}
