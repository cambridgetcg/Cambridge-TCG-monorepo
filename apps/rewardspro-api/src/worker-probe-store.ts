...
export class PostgresWorkerProbeStore implements WorkerProbeStore {
  ...
  async create(probeId: string, lifetimeSeconds: number): Promise<void> {
    await this.pool.query(
      `INSERT INTO rp_worker_probe (id, expires_at)
  ...
  async delete(probeId: string): Promise<void> {
    await this.pool.query("DELETE FROM rp_worker_probe WHERE id = $1", [probeId]);
  }

  async deleteExpired(): Promise<void> {
    await this.pool.query("DELETE FROM rp_worker_probe WHERE expires_at <= now()");
  }
}
