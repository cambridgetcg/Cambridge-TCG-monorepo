    ...
    >
      ...
      {open && (
        ...
        <div
          ...
          onMouseLeave={scheduleClose}
        >
          <div className="w-[680px] max-w-[calc(100vw-2rem)] bg-surface border border-border-subtle rounded-lg shadow-mat">
  ...
  return (
    <li>
      <Link
        href={item.href}
        onClick={onSelect}
        className="block group rounded-md px-2 py-1.5 -mx-2 hover:bg-surface-subtle transition"
      >
  ...
  const styles =
    kind === "live"
      ...
      : kind === "beta"
        ...
        : "bg-surface-subtle text-ink-faint border-border-subtle";
  return (
    <span className={`inline-block text-[9px] font-semibold uppercase tracking-wider px-1.5 py-0.5 rounded border ${styles}`}>
      {kind}
    </span>
  );
}
