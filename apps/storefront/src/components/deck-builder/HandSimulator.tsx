...
export default function HandSimulator({
  ...
  return (
    ...
    >
      <div
        className="bg-surface border border-border-subtle rounded-xl p-6 max-w-3xl w-full shadow-2xl max-h-[90vh] overflow-y-auto"
      ...
      >
        ...
        {tooSmall && (
          <div className="mb-4 bg-amber-900/30 border border-amber-700/40 text-accent-strong rounded-lg px-4 py-3 text-sm">
        ...
        <div className="mb-6">
          <div className="flex items-baseline justify-between mb-2">
            ...
            <div className="flex gap-2">
              <button
                onClick={mulligan}
                disabled={tooSmall}
                className="text-xs bg-surface-elevated hover:bg-neutral-700 border border-border-strong rounded px-3 py-1.5 transition-colors disabled:opacity-40"
              >
                Mulligan
              </button>
              <button
                onClick={redraw}
                disabled={tooSmall}
                className="text-xs bg-accent hover:bg-accent-strong text-black font-bold rounded px-3 py-1.5 transition-colors disabled:opacity-40"
              >
                Shuffle &amp; Draw
              </button>
            </div>
          ...
          <div className="grid grid-cols-5 gap-2">
            {hand.map((card, i) => (
              <div key={`${card.sku}-${i}`} className="relative aspect-[5/7] rounded-md overflow-hidden bg-surface-elevated border border-border-strong">
                {card.image_url ? (
                  <Image
                    src={card.image_url}
                    alt={card.name}
            ...
            {Array.from({ length: Math.max(0, handSize - hand.length) }).map((_, i) => (
              <div key={`empty-${i}`} className="aspect-[5/7] rounded-md bg-surface-elevated/40 border border-dashed border-border-subtle" />
            ))}
          </div>
        </div>
        ...
        {entries.length > 0 && (
          <div className="mb-6">
            ...
            <div className="max-h-40 overflow-y-auto bg-page/40 border border-border-subtle rounded-lg p-2">
        ...
        <div>
          <button
            ...
            className="w-full bg-surface-elevated hover:bg-neutral-700 border border-border-strong text-ink font-semibold rounded-lg py-3 transition-colors disabled:opacity-50 text-sm"
          >
            {simRunning
              ? "Running 10,000 sims..."
              : targetSkus.size === 0
          ...
          {simResult && (
            <div className="mt-4 bg-page/40 border border-border-subtle rounded-lg p-4 space-y-3">
