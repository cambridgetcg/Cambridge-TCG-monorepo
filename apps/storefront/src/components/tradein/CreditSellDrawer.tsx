...
export default function CreditSellDrawer() {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div className="relative bg-page w-full max-w-md h-full flex flex-col border-l border-border-subtle">
        {/* Header */}
        ...
        {items.length > 0 && (
          <div className="px-4 py-4 border-t border-border-subtle space-y-3">
            ...
            <button
              ...
              disabled={submitting}
              className="w-full py-3 bg-purple-500 text-ink font-bold rounded-lg hover:bg-purple-400 transition disabled:opacity-50"
            >
              {submitting ? "Submitting..." : `Submit ${totalItems} Card${totalItems !== 1 ? "s" : ""} for Credit`}
            </button>
            <button
              onClick={clearCart}
              className="w-full py-2 text-sm text-ink-faint hover:text-red-400 transition"
            >
              Clear all
            </button>
          </div>
