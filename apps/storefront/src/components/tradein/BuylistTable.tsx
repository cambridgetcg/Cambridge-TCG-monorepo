...
function wantIndicator(cashWant: number, creditWant: number, mode: "cash" | "credit") {
  ...
  if (cashWant >= 1) {
    return <span className="w-2 h-2 rounded-full bg-accent-strong inline-block" title="Limited" />;
  }
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        ...
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 bg-surface border border-border-subtle rounded-lg text-sm text-ink placeholder-neutral-500 focus:outline-none focus:ring-2 focus:ring-accent/50"
          />
        </div>
        ...
        <select
          ...
          className="px-3 py-2.5 bg-surface border border-border-subtle rounded-lg text-sm text-ink focus:outline-none focus:ring-2 focus:ring-accent/50"
        >
              ...
              <tbody>
                {filtered.map((item) => {
                  ...
                  return (
                    <tr
                      key={item.sku}
                      className="border-b border-border-subtle/50 hover:bg-surface/50 transition"
                    >
                      <td className="py-2 px-2">
                        <div className="relative w-10 h-14 rounded overflow-hidden bg-surface-elevated shrink-0">
                          {item.image_url ? (
                            <Image
                              src={item.image_url}
                              alt={item.name}
                      ...
                      <td
                        className={`py-2 px-2 text-right text-sm font-medium ${
                          mode === "cash" ? "text-accent-strong" : "text-ink-muted"
                        }`}
                      >
                      ...
                      <td
                        className={`py-2 px-2 text-right text-sm font-medium ${
                          mode === "credit" ? "text-accent-strong" : "text-ink-muted"
                        }`}
                      >
          ...
          <div className="md:hidden space-y-3">
            {filtered.map((item) => {
              ...
              return (
                ...
                >
                  <div className="relative w-16 h-22 rounded-lg overflow-hidden bg-surface-elevated shrink-0">
                    {item.image_url ? (
                      <Image
                        src={item.image_url}
                        alt={item.name}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex gap-4 mt-2">
                      <div>
                        ...
                        <p className={`text-sm font-medium ${mode === "cash" ? "text-accent-strong" : "text-ink-muted"}`}>
                          {item.cash_price > 0 ? formatPrice(item.cash_price) : "—"}
                        </p>
                      </div>
                      <div>
                        <p className="text-[10px] text-ink-faint uppercase">Credit</p>
                        <p className={`text-sm font-medium ${mode === "credit" ? "text-accent-strong" : "text-ink-muted"}`}>
                          {item.credit_price > 0 ? formatPrice(item.credit_price) : "—"}
                        </p>
                      </div>
                    </div>
