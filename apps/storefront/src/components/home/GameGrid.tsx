...
export default function GameGrid({ games }: { games: GameItem[] }) {
  return (
    <section className="max-w-7xl mx-auto px-4 py-16">
      <h2 className="text-2xl font-bold mb-8 text-ink">Shop by Game</h2>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {games.map((g) => (
          <Link
            key={g.code}
            href={`/catalog?game=${g.slug}`}
            className="group relative aspect-square rounded-2xl overflow-hidden bg-surface hover:ring-2 ring-emerald-500 transition-all"
          >
            <Image
            ...
            <div className="absolute inset-0 flex items-end justify-center pb-6">
              <span className="text-ink font-bold text-lg drop-shadow-lg tracking-wide">
                {g.name}
              </span>
            </div>
          </Link>
