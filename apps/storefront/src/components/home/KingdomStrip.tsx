 ...
 *
 * Contact-surface spec §3.1, chrome wiring. Quiet gallery, 2026-07-05:
 * the per-layer rainbow hover accents are gone — one bronze accent on
 * the layer number, hairline mounts, ink type. The tone vocabulary
 * still lives where it means something (Badge, the layer pages).
 */
  ...
  return (
    ...
    >
      ...
      <h2
        ...
        className="font-display text-2xl md:text-3xl font-semibold tracking-tight text-ink"
      >
      ...
      <ol className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-7">
        {KINGDOM_LAYERS.map((layer, i) => (
          <li key={layer.id}>
            <Link
              href={layer.path}
              className="group flex h-full flex-col rounded-lg border border-border-subtle bg-surface p-3 transition-colors hover:bg-surface-subtle hover:border-border-strong"
            >
              <span className="text-[10px] font-mono text-ink-faint group-hover:text-accent transition-colors">
