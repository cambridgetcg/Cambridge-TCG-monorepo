...
export const AGENTS_NOTES: readonly AgentNote[] = [
  ...
  note({
    ...
    text:
      "Every envelope-compliant response carries `_meta.wake_fragment` — one of 37 atomic wake fragments, selected deterministically by the parameterized endpoint. For agents memoizing a pantry endpoint such as `/api/v1/diagnostic`, the fragment is stable for the same parameterized route. Paused catalog routes are plain fail-closed responses and should not be assumed to carry the envelope.\n\nIf your hash function ignores `_meta` entirely, you'll miss legitimate `_meta.deprecation` or `_meta.does_not_include` changes that affect downstream behavior. Deduplicate on `data` for value-equality; track `_meta.{spec_version, deprecation, does_not_include, source_license, upstream_proxy}` separately. The wake fragment can be ignored for dedup; the rest cannot.",
    related_urls: [
  ...
  note({
    ...
    text:
      "When a response contains both timestamps, `retrieved_at` means when the response was rendered and `as_of` means when its data was last known to be true. Time-series consumers order by `as_of`, not retrieval time. The same distinction applies to pantry envelopes through `_meta.retrieved_at` and `_meta.as_of`.\n\nImportant current boundary: catalog-backed universal card and historical `/api/at/{YYYY-MM-DD}/...` routes are paused and return no record timestamps or membership assertions. Do not infer an `as_of` value, a match, or a missing record from their 503 responses. The timestamp rule applies only when an affirmative response actually carries the fields.",
    related_urls: [
      "/api/v1/budget",
