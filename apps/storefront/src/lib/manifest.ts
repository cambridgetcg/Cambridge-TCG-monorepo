...
export const MANIFEST: Manifest = {
  ...
  description:
    "Cambridge TCG is a collectors' market and a public data commons. The market is peer-to-peer — the platform facilitates, records, and witnesses, and holds no position in its own market (collectors-first decision, 2026-07-06). Cambridge-authored schemas and explicitly first-party datasets may be CC0. Upstream-derived card fields retain their source rights; mixed catalog responses are NOASSERTION. Public reachability is not an open license. This manifest is the directory of what's on offer to any participant who wants to inspect or consume the substrate — partners, researchers, agents, archivists, sister platforms, federation clients, and unfamiliar readers. It names what is real, who can take part, what is offered, how to receive it, and how to inspect each boundary.",
  ...
  resources: {
    discovery: [
      ...
      { id: "storefront.prices.per_set", description: "Per-set price guide — parametric route under sister's kingdom-084. /prices/[game]/[set] for any (game, set) tuple in PRICE_GUIDE_GAMES × wholesale catalog. Renders the full card list with rarity badges + GBP reference-price columns (labelled reference, never an offer). Card name + number link through to per-card detail page.",
        ...
        cosmology_axes: ["value", "knowledge"], methodology_url: "/methodology/pricing",
        since: "2026-05-13" },
      { id: "storefront.prices.per_card", description: "Per-card price-guide detail — /prices/[game]/[set]/[number]. The source panel reports CardRush as observed where confirmed, TCGplayer as blocked, and Cardmarket as planned. It does not turn planned or blocked sources into apparent coverage. Resolves card by (game, set, number) and renders Product + BreadcrumbList JSON-LD.",
      ...
      { id: "storefront.api.prices.per_card", description: "JSON sibling of /prices/[game]/[set]/[number]. Reuses loadCardState. Returns mixed card metadata plus explicit observed, planned, and blocked source states. Aggregate rights NOASSERTION; Cambridge-authored envelope and schema remain CC0 separately.",
        ...
        cosmology_axes: ["value", "knowledge", "identity"], methodology_url: "/methodology/cross-source-pricing",
        since: "2026-05-14" },
      { id: "storefront.api.sets.checklist", description: "Public set checklist — the full card list of one set (card_number / variant / sku / card_name / rarity / image_url) + set metadata with total_cards_declared vs total_cards_actual named apart (substrate-honest when the mirror is mid-import). No user data — the signed-in twin (portfolio-intersected) lives at /api/account/sets/[code]. Paginated honestly: ?offset/&limit (max 500) + next_link + total. Every emitted link percent-encodes its segments (card numbers may contain '/'). Data-pantry envelope (CC0). Collector-facing data surface, 2026-07.",
        ...
        since: "2026-07-12" },
      { id: "storefront.api.cards.history", description: "Public reference-price history for one SKU — four windows (7d/30d/90d/365d) of daily {captured_on, spot_gbp, best_bid_gbp, best_ask_gbp} from card_price_history, via the same loadPriceHistory composer that feeds /cards/[sku]/market. Every payload carries the collectors-first price_basis framing (in data AND _meta): spot is a labelled reference price, never an offer — the house holds no market position (docs/decisions/2026-07-06-collectors-first.md). Data-pantry envelope (CC0). Collector-facing data surface, 2026-07.",
    ...
    methodology: [
      ...
      { id: "storefront.sold_comps.sku", description: "Per-SKU face of /api/v1/sold-comps — CC0 anonymised aggregate sold prices for one canonical card, from the kingdom's own completed trades + settled auctions. Same two safety rings (PII-stripped view + K>=5 aggregation). Honest absence: a card not yet sold >=5 times returns empty buckets with a plain coverage note, never a fabricated price. See /methodology/data-intentions.",
        ...
        cosmology_axes: ["value", "time", "transaction"], methodology_url: "/methodology/data-intentions",
        since: "2026-07-11" },
      { id: "storefront.datasets.json", description: "The dataset catalog: the open data commons indexed AS DATASETS (not just endpoints). Lists every dataset the platform publishes — first-party sold comps, catalogue coverage, the source-rights registry, known gaps, the agent ladder, and the bulk card catalogue — each carrying its TRUE licence (first-party operational data CC0; the mixed card catalogue NOASSERTION, never relabelled). The CC0 envelope covers only our own authored dataset descriptions; each dataset's own licence rides in-band. Add ?format=jsonld for a bare schema.org/DataCatalog graph aimed at Google Dataset Search + AI crawlers. Completes docs/connections/the-finding.md Plant C. Registered in the redistribution audit so this CC0 surface stays airtight. _meta.source_license=['cc0'].",
        ...
        since: "2026-07-12" },
      { id: "storefront.datasets.human", description: "Human-readable face of /api/v1/datasets — the commons as datasets, one card per dataset with its licence badge, coverage, fields, and where to get it. Carries an inline schema.org/DataCatalog block so dataset-search indexers and AI crawlers discover every dataset in one page load. Sister to /data (which indexes endpoints).",
      ...
      { id: "storefront.catalog.jsonl", description: "Bulk catalog export. Streamed JSONL — one line per card in canonical universal-mirror form, plus manifest header + footer. Aggregate rights are NOASSERTION because the mirror does not yet preserve field-level upstream rights; Cambridge-authored encoding/schema remain CC0 separately. Caps at 50k rows per request. Vercel CDN gzips automatically.",
        ...
        since: "2026-05-14" },
      { id: "wholesale.prices.sources", description: "Reviewed-source view of one card on its latest (or specified) snapshot date. Today only CardRush may be served. Stored rows from any other source remain withheld until a publication rule is reviewed; the schema alone does not grant rights.",
      ...
      { id: "storefront.search.cards", description: "Card-number resolver — turn (game, query) into one or more canonical SKU candidates with confidence labels. Three input shapes: 'OP01-001' (set+number), '001' (number alone; fuzzy), 'op-op01-001-ja' (full canonical). Returns matches array + summary { count, best_confidence, distinct_set_number_buckets, ambiguous }. The response includes upstream-derived names and catalog fields, so aggregate rights are NOASSERTION; Cambridge-authored matching structure remains CC0 separately.",
        ...
        since: "2026-05-14" },
      { id: "storefront.cards.everything", description: "The composer — given a canonical SKU, returns mixed card metadata, reviewed current price rows, CardRush history summary, siblings, and a labelled reference price. TCGplayer is never fetched or emitted while blocked. Aggregate card-metadata rights are NOASSERTION; source boundaries ride in _meta.source_license.",
    ...
    joy: [
      ...
      { id: "storefront.buy_the_kingdom",
        description: "GET /api/v1/buy-the-kingdom → HTTP 402 Payment Required: 'not for sale.' Cambridge-authored methodology and the envelope contract are CC0; upstream-derived card fields retain their source rights and mixed catalog responses are NOASSERTION. Teaches that public access is not blanket relicensing and points at /api/v1/manifest for the exact boundaries.",
