  ...
  resources: {
    discovery: [
      ...
      { id: "storefront.api.prices.per_set", description: "JSON sibling of /prices/[game]/[set]. Reuses loadSetState. Returns game + set meta + full card list with per-card API + HTML paths. Data-pantry envelope (CC0).",
        ...
        cosmology_axes: ["value", "knowledge"], methodology_url: "/methodology/cross-source-pricing",
        since: "2026-05-14" },
      { id: "storefront.api.prices.per_card", description: "JSON sibling of /prices/[game]/[set]/[number]. Reuses loadCardState. Returns game + set + card meta + cross_source_signals[] (per-source arrival state + license tier + signed-in path) + _links to math-mirror / market / parent surfaces. Data-pantry envelope (CC0). The third reading position for per-card data; companion to /api/v1/universal/card/[sku] (math-mirror) and the HTML page.",
    ...
    market: [
      ...
      { id: "storefront.auctions", description: "Browse + bid on auctions.",
        ...
        since: "2026-04-15" },
      { id: "storefront.tradein.quote", description: "In-flight trade-in settlement — NOT intake (closed, kingdom-101): admin issues quotations for already-received submissions (POST, admin-gated); customers accept or decline a pending quote (PATCH). Drains the desk's remaining obligations.",
        host: "storefront", path: "/api/tradein/quote", methods: ["POST", "PATCH"],
        modalities: ["json"], auth: "user", provenance: "live",
        cosmology_axes: ["value", "transaction"],
        since: "2026-03-15" },
      { id: "storefront.quotes", description: "Custom-quote desk records. Intake CLOSED (regulator pivot, kingdom-101) — POST returns 410; GET serves admin reads for in-flight quotes, which are honored in full.",
