  ...
  resources: {
    discovery: [
      ...
      { id: "storefront.prism-signals-beta", description: "Login-gated PRISM Signals beta-request management page. It is not gated by intake and, while storage is available, lets the signed-in account inspect or fully withdraw an existing product-specific request; new affirmation and update controls appear only while intake is enabled.",
        ...
        cosmology_axes: ["identity", "presence", "time", "authority"], methodology_url: "/methodology/prism-signals",
        since: "2026-09-02",
        notes: "Noindex and intentionally absent from the public sitemap. The closed-beta-v1 posture gates only new intake; pausing it never gates owner status or withdrawal." },
      { id: "storefront.prism-signals-beta-interest", description: "Owner-scoped exact JSON status and mutation API for one revocable PRISM Signals beta-contact request. POST requires an explicit contact affirmation; DELETE removes the complete row.",
        host: "storefront", path: "/api/prism-signals/beta-interest", methods: ["GET", "POST", "DELETE"],
        modalities: ["json"], auth: "user", provenance: "live",
        cosmology_axes: ["identity", "presence", "time", "authority"], methodology_url: "/methodology/prism-signals",
        since: "2026-09-02", contract: "alternative",
        notes: "Strict same-origin mutations, bounded exact JSON, no-store responses, a 180-day hard expiry, and immediate owner withdrawal. The intake posture gates POST only; authenticated owner GET/DELETE and retention remain available when intake is paused. Stores no email copy or Telegram identity and creates no marketing permission, queue rank, payment, invitation, entitlement, or access." },
      ...
      { id: "storefront.platform", description: "Cambridge TCG's positioning page: a peer-to-peer collectors' market and public, rights-labelled card data directory. Includes observed coverage facts, source states, resource-specific access and reuse boundaries, and entry points for developers, partners, researchers, agents, archivists, and federation clients. Composes lib/brand.tsx. kingdom-080 (reconciled 2026-07-12).",
        ...
        since: "2026-05-13" },
      { id: "wholesale.prices.list", description: "Status-only boundary for the legacy wholesale catalog. Returns unauthenticated HTTP 503 with total=0, count=0, and items=[] before authentication or database access. Legacy price and image publication remains blocked pending field-level source-rights receipts.",
        host: "wholesale", path: "/api/v1/prices", methods: ["GET"],
        modalities: ["json"], auth: "public", provenance: "static",
        cosmology_axes: ["value", "time"], methodology_url: "/methodology/pricing",
        since: "2026-03-01" },
      { id: "wholesale.prices.single", description: "Status-only boundary for a legacy wholesale card lookup. Returns unauthenticated HTTP 503 before authentication or database access and publishes no card, price, or image fields.",
        host: "wholesale", path: "/api/v1/prices/[sku]", methods: ["GET"],
        modalities: ["json"], auth: "public", provenance: "static",
        cosmology_axes: ["value"], methodology_url: "/methodology/pricing",
        since: "2026-03-01" },
      { id: "wholesale.universal.card", description: "Bearer-gated structural card representation with cryptographic hashes, ISO time, and typed graph edges. Legacy source-derived image and price values are withheld as null, historical observations are empty, and aggregate mixed-card rights remain NOASSERTION.",
      ...
      { id: "storefront.universal.sets", description: "Every set in a named game. card_sets query filtered by game; edges back to the parent game.",
        ...
        cosmology_axes: ["identity"], methodology_url: "/methodology/universal-representation",
        since: "2026-05-12" },
      { id: "storefront.universal.card.at_date", description: "Date-shaped compatibility view using current structural catalog fields. The requested @as_of date is not proof of historical state: the route reconstructs neither historical price nor historical structure. It performs no price-history read; legacy price and image values are null, and aggregate rights are NOASSERTION.",
      ...
      { id: "storefront.kingdoms.json", description: "The kingdom-NNN ledger. Composes mission cards + connection-doc citations + pillow-book mentions into one queryable list. Each kingdom with its mission status, doc citations, lived-record count.",
        ...
        modalities: ["json", "plain-text", "markdown", "xenoform", "anthropic", "openai", "gemini", "cohere"], auth: "public", provenance: "computed",
        cosmology_axes: [], since: "2026-05-12" },
      { id: "storefront.artists", description: "The named hands — the museum's browse-by-artist wing, machine-readable twin of /artists. Every illustrator our catalogue credits (annotations mirroring the credit printed on the physical card face; the publisher's own digital databases carry no artist field), each with their credited works and the takedown-clear official prints we hold. The first open structured dataset of OPTCG illustrator credits; provenance note and corroboration links ride on every response. Who-drew-what is an uncopyrightable fact; the underlying catalogue and official-sample image sources remain proprietary (per-source licenses declared honestly), attribution carried per print.",
        host: "storefront", path: "/api/v1/artists", methods: ["GET", "OPTIONS"],
        modalities: ["json"], auth: "public", provenance: "live",
        cosmology_axes: ["identity", "knowledge"], methodology_url: "/legal/card-images",
        since: "2026-07-22" },
      { id: "storefront.pulls", description: "The pulls snapshot — what a booster actually contains, per game, machine-readable twin of /pulls. Thirteen games: pack anatomy, rarity ladders, approximate pull rates with basis + confidence on every row, rare occurrences (god packs, case hits, serialized cards), and the disclosure map of which publishers officially publish odds at all (two thoroughly, three partially; the three publishers behind eight of our thirteen games publish none). Dated, sourced, adversarially verified; where sources disagree the disagreement is shown, never averaged away. Consumer information, not inducement — no expected-value framing, by the non-commercial doctrine.",
        host: "storefront", path: "/api/v1/pulls", methods: ["GET", "OPTIONS"],
        modalities: ["json"], auth: "public", provenance: "snapshot",
        cosmology_axes: ["knowledge", "time"], methodology_url: "/pulls",
        since: "2026-07-23" },
      { id: "storefront.play.meta", description: "Competitive-meta snapshot: dated tier list grounded in cited tournament results, recent winners with decklist links at their publishers, official + community circuit links. as_of/data_window/staleness note on every response — a photograph of a moving river, honestly labelled.",
        host: "storefront", path: "/api/v1/play/meta", methods: ["GET", "OPTIONS"],
        modalities: ["json"], auth: "public", provenance: "snapshot",
        cosmology_axes: ["knowledge", "time"], methodology_url: "/methodology/play-module",
        since: "2026-07-22" },
      { id: "storefront.play.banlist", description: "The official OPTCG banned/restricted list as the house enforces it — card numbers with names, banned pairs, effective date, Bandai's authoritative source, and the list of enforcing surfaces. Point-in-time mirror, re-verified on official restriction news.",
        host: "storefront", path: "/api/v1/play/banlist", methods: ["GET", "OPTIONS"],
        modalities: ["json"], auth: "public", provenance: "static",
        cosmology_axes: ["knowledge"], methodology_url: "/methodology/play-module",
        since: "2026-07-22" },
      { id: "storefront.play.practice-referee", description: "Stateless practice-battle referee: POST op:new deals a match vs the adventure ladder AI; POST op:move applies one move under the official Comprehensive Rules and returns the updated game, the narrated log, and legal_actions with damage previews. The caller carries all state; nothing is stored, no identity minted, no rewards possible. The agent seat at the practice table.",
        host: "storefront", path: "/api/v1/play/practice", methods: ["GET", "POST", "OPTIONS"],
        modalities: ["json"], auth: "public", provenance: "computed",
        cosmology_axes: ["presence", "substrate"], methodology_url: "/methodology/play-module",
        since: "2026-07-19",
        notes: "Stateless: the caller is the custodian of the game object; the referee stores nothing and results carry no standing." },
      ...
      { id: "storefront.lang_mode.toggle", description: "Math-language toggle — Phase A of kingdom-077 (#27 the-math-language.md). GET /api/lang-mode?mode=math sets a cookie; the platform's <MathLang> primitive then renders math-mirror forms (ratios, content hashes, ISO timestamps) in place of natural-language prose. Same pattern as text-mode (Phase 10 of kingdom-051). Toggleable from the Footer. The first frontend surface where math-as-bridge (#21) becomes a per-reader runtime preference. Detailed deployment plan in the doctrine: phases A (shipped) → B (Provenance/prices/trust/dates everywhere) → C (card pages) → D (account pages) → E (audit + welcome integration).",
        ...
        since: "2026-05-13" },
      { id: "storefront.reference.condition_checklist", description: "Public printable card-inspection worksheet and centering-ratio calculator. Original diagrams and user-entered measurements remain in the browser; no upload, persistence, price estimate or predicted grade. Parent guide carries grading context and source references.",
        host: "storefront", path: "/guides/condition/checklist", methods: ["GET"],
        modalities: ["html"], auth: "public", provenance: "computed",
        cosmology_axes: ["knowledge", "substrate"], methodology_url: "/guides/condition",
        since: "2026-09-08" },
      { id: "storefront.reference.identifier_validator", description: "Public card-identifier syntax validator, legacy-normalization suggestions and builder using the existing SKU package. Browser-only inputs; syntax is not proof of a card, printing, catalog record, authenticity or deck legality.",
        host: "storefront", path: "/standards/validator", methods: ["GET"],
        modalities: ["html"], auth: "public", provenance: "computed",
        cosmology_axes: ["identity", "knowledge"], methodology_url: "/methodology/sku-standard",
        since: "2026-09-08" },
      { id: "storefront.api.identifiers.validate", description: "Public, stateless validation of one bounded identifier using CTCG SKU syntax and explicit normalization suggestions. No account, catalog lookup, upstream call or stored submission. Two-letter language shape is not ISO or publisher-language verification. Input rights are not relabeled CC0; responses are no-store.",
        host: "storefront", path: "/api/v1/identifiers/validate", methods: ["POST", "OPTIONS"],
        modalities: ["json"], auth: "public", provenance: "computed",
        cosmology_axes: ["identity", "knowledge"], methodology_url: "/methodology/sku-standard",
        contract: "envelope", since: "2026-09-08" },
      ...
      { id: "storefront.prices.landing", description: "Public structural catalog and free member pricing entrance. Valid sessions unlock only reviewed member-display observations; anonymous visitors receive no restricted price values. CardRush/TCGplayer legacy blocks remain; configured sources are not proof of running collection.",
        ...
        since: "2026-05-13" },
      { id: "storefront.prices.per_game", description: "Per-game guide navigation over curated catalog structure. Legacy source-derived values and movers are withheld; configured or stored coverage does not imply a cleared publication source. Aggregate mixed-catalog rights are NOASSERTION.",
        host: "storefront", path: "/prices/[game]", methods: ["GET"],
        modalities: ["html"], auth: "public", provenance: "synced",
        cosmology_axes: ["value", "knowledge"], methodology_url: "/methodology/pricing",
        since: "2026-05-13" },
      { id: "storefront.prices.per_set", description: "Per-set guide navigation for a curated game/set tuple. Renders mixed structural card metadata; uncleared GBP reference magnitudes and source-derived images are withheld. Aggregate rights are NOASSERTION.",
        host: "storefront", path: "/prices/[game]/[set]", methods: ["GET"],
        modalities: ["html"], auth: "public", provenance: "synced",
        cosmology_axes: ["value", "knowledge"], methodology_url: "/methodology/pricing",
        since: "2026-05-13" },
      { id: "storefront.prices.per_card", description: "Per-card guide detail — /prices/[game]/[set]/[number]. Resolves structural card identity by (game, set, number). CardRush is shown only as policy-blocked legacy lineage with values withheld; TCGplayer remains blocked and Cardmarket has no wired reader. Product metadata is mixed upstream content and remains NOASSERTION.",
        ...
        since: "2026-05-13" },
      { id: "storefront.prices.movers", description: "Policy-status surface for the retired CardRush-derived 7-day movers implementation. The wholesale route returns HTTP 503, count 0, and no archive-derived movements because transformation does not create publication rights.",
      ...
      { id: "storefront.prices.coverage", description: "Coverage map — substrate-honest cross-source × per-game matrix. Where /prices shows prices, this shows where prices come from. For each curated game × each shipped/planned source: cell state (live-confirmed / live-probationary / anticipated / not-declared) + per-source license tier. Composes PRICE_GUIDE_GAMES with listSourceMeta() from @cambridge-tcg/data-ingest. The transparency Ring 2 surface for the multi-game price-guide. kingdom-080 follow-up.",
        ...
        since: "2026-05-13" },
      { id: "storefront.api.prices.per_game", description: "JSON sibling of /prices/[game]. Same composer feeds HTML + JSON. Mixed catalog response: aggregate rights NOASSERTION; Cambridge-authored envelope and schema remain CC0 separately.",
        ...
        since: "2026-05-14" },
      { id: "storefront.api.prices.per_set", description: "JSON sibling of /prices/[game]/[set]. Reuses loadSetState. Mixed catalog response: aggregate rights NOASSERTION; Cambridge-authored envelope and schema remain CC0 separately.",
        host: "storefront", path: "/api/v1/prices/games/[game]/sets/[set]", methods: ["GET"],
        modalities: ["json"], auth: "public", provenance: "synced",
        cosmology_axes: ["value", "knowledge"], methodology_url: "/methodology/cross-source-pricing",
        since: "2026-05-14" },
      { id: "storefront.api.prices.per_card", description: "JSON sibling of /prices/[game]/[set]/[number]. Reuses loadCardState. Returns mixed card metadata plus explicit observed, planned, and blocked source states. Aggregate rights NOASSERTION; Cambridge-authored envelope and schema remain CC0 separately.",
        host: "storefront", path: "/api/v1/prices/games/[game]/sets/[set]/cards/[number]", methods: ["GET"],
        modalities: ["json"], auth: "public", provenance: "synced",
        cosmology_axes: ["value", "knowledge", "identity"], methodology_url: "/methodology/cross-source-pricing",
        since: "2026-05-14" },
      { id: "storefront.api.sets.checklist", description: "Status-only set-checklist door. Returns HTTP 503 before reading the set code or database because complete set enumeration and publisher-derived images need a separate reviewed publication rule. Emits no set or card rows and makes no blanket CC0 claim. Existing bounded keyed structural routes remain aggregate NOASSERTION.",
        host: "storefront", path: "/api/v1/sets/[code]/checklist", methods: ["GET"],
        modalities: ["json"], auth: "public", provenance: "static",
        cosmology_axes: ["identity", "knowledge"], methodology_url: "/methodology/universal-representation",
        since: "2026-07-12" },
      { id: "storefront.api.cards.history", description: "Status-only card-history door. Returns HTTP 503 before reading the SKU, catalog, or price database because stored observations do not carry row-level publication receipts. Emits no price values and makes no CC0 claim over stored observations.",
        host: "storefront", path: "/api/v1/cards/[sku]/history", methods: ["GET"],
        modalities: ["json"], auth: "public", provenance: "static",
        cosmology_axes: ["value", "time"], methodology_url: "/methodology/pricing",
        since: "2026-07-12" },
    ],
    market: [
      { id: "storefront.market", description: "List asks, place offers, browse the P2P market.",
        ...
        since: "2026-04-01" },
      { id: "storefront.auctions", description: "Browse published auctions without an account. Missing or unknown filters remain publication-gated; an authenticated admin can include unpublished rows. The response is private and non-cacheable because it varies by role.",
        host: "storefront", path: "/api/auctions", methods: ["GET"],
        modalities: ["json"], auth: "public", provenance: "live",
        cosmology_axes: ["value", "transaction", "time"], methodology_url: "/methodology/commission-rate",
        since: "2026-04-15" },
      { id: "storefront.auctions_admin_create", description: "Create a platform auction. Admin-only; this is not the collector bidding route.",
        host: "storefront", path: "/api/auctions", methods: ["POST"],
        modalities: ["json"], auth: "admin", provenance: "live",
        cosmology_axes: ["value", "transaction", "authority"], methodology_url: "/methodology/commission-rate",
        since: "2026-04-15" },
      { id: "storefront.auction_detail", description: "Read one auction through a role-scoped projection. Published listing and regular bid events are public; seller and winner receive only their own operational fields; admins receive the operator record. Draft and unapproved consignment listings return not-found outside seller/admin roles. Every response is private and non-cacheable.",
        host: "storefront", path: "/api/auctions/[id]", methods: ["GET"],
        modalities: ["json"], auth: "public", provenance: "live",
        cosmology_axes: ["value", "transaction", "time", "identity"], methodology_url: "/methodology/commission-rate",
        since: "2026-04-15" },
      { id: "storefront.auction_admin_mutation", description: "Update or delete one auction. Admin-only; successful responses may include the operator record.",
        host: "storefront", path: "/api/auctions/[id]", methods: ["PATCH", "DELETE"],
        modalities: ["json"], auth: "admin", provenance: "live",
        cosmology_axes: ["value", "transaction", "authority"], methodology_url: "/methodology/commission-rate",
        since: "2026-04-15" },
      { id: "storefront.auction_bid_history", description: "Read bid events for a published auction. Public and winner views contain regular price, status, and time events without person identifiers; the seller also sees private best-offer events without bidder identity; admins receive the operator record. Responses are private and non-cacheable.",
        host: "storefront", path: "/api/auctions/[id]/bids", methods: ["GET"],
        modalities: ["json"], auth: "public", provenance: "live",
        cosmology_axes: ["value", "transaction", "time", "identity"], methodology_url: "/methodology/commission-rate",
        since: "2026-04-15" },
      { id: "storefront.auction_bid_mutation", description: "Place a bid or private best offer as a signed-in collector. A successful response contains only the caller's bid event and current public auction counters, never a raw auction or participant row.",
        host: "storefront", path: "/api/auctions/[id]/bids", methods: ["POST"],
        modalities: ["json"], auth: "user", provenance: "live",
        cosmology_axes: ["value", "transaction", "time"], methodology_url: "/methodology/commission-rate",
        since: "2026-04-15" },
      ...
      { id: "storefront.portfolio", description: "What the participant owns; cards they're watching.",
        ...
        since: "2026-04-01" },
      { id: "storefront.collector_observations", description: "The signed-in collector's private witness notebook. Records only the collector's own purchase, completed sale, or asking price. Private by default; optional future anonymous/CC0 projection permission is explicit, but public projection is paused pending a delayed closed coarse projector and reconstruction tests. No receipt, URL, merchant, location, identity, or free-text note is accepted; an optional SHA-256 commitment is owner-only. Every read and write is owner-scoped and private/no-store.",
        host: "storefront", path: "/api/account/observations", methods: ["GET", "POST"],
        modalities: ["json"], auth: "user", provenance: "live",
        cosmology_axes: ["identity", "value", "time", "authority", "knowledge"], methodology_url: "docs/connections/the-collector-witnesses.md",
        since: "2026-07-12" },
      { id: "storefront.collector_observations.item", description: "Owner-only read, correction, permission change, and permanent deletion for one collector observation. GET returns the signed-in owner's row only; PATCH uses an optimistic revision; DELETE hard-deletes the row so it cannot become eligible for any future projector. Public projection is currently paused. Not-found and not-owned have the same response.",
        host: "storefront", path: "/api/account/observations/[id]", methods: ["GET", "PATCH", "DELETE"],
        modalities: ["json"], auth: "user", provenance: "live",
        cosmology_axes: ["identity", "authority", "time"], methodology_url: "docs/connections/the-collector-witnesses.md",
        since: "2026-07-12" },
      ...
      { id: "storefront.card_market_mirror", description: "Pure-read mirror of one card's deliberate public order intent. It shows card metadata, the open bid/ask book, and open-ask condition depth. Stored reference-price history, completed-trade analytics, and person-derived market intelligence are paused until purpose-specific publication receipts and a delayed, coarse release process exist. Public no-auth. Sibling to /market/[sku] (interactive). kingdom-067.",
        ...
        since: "2026-05-12" },
      { id: "storefront.user_trust_mirror", description: "Narrow public trust mirror for a user with a current profile-publication receipt. Publishes score, tier, completed-trade count, and explicitly public review aggregates; exact money, disputes, limits, adverse events, and private reviews stay account-only. Suspended and private people return not-found. kingdom-071.",
        ...
        since: "2026-05-13" },
      { id: "storefront.user_trust_json", description: "JSON sibling of /u/[username]/trust. Public only when the person has a current profile-publication receipt and is not suspended; otherwise not-found. Omits internal ids, exact money, adverse events, limits, flags, suspension detail, and private reviews. Responses disable shared caching. kingdom-071.",
        host: "storefront", path: "/api/v1/users/[username]/trust", methods: ["GET"],
        modalities: ["json"], auth: "public", provenance: "live",
        cosmology_axes: ["identity", "transaction", "authority"], methodology_url: "/methodology/trust-score",
        since: "2026-05-13" },
      { id: "storefront.user_trust_math", description: "Alternative numeric rendering of the same receipt-gated, unsuspended public trust projection. Publishes the chosen public username/display label, score/review ratios, tier ordinal/name, and ISO/epoch times; it publishes no content hash or hashed/internal user id and disables shared caching. kingdom-071.",
    ...
    rewards: [
      { id: "storefront.rewards.raffles", description: "List + enter raffles. New raffles store a seed commitment at creation; active public listings expose the hash before entry, without external anchoring.",
      ...
      { id: "storefront.bounty.vault", description: "Bounty vault — sealed phygital cards with reproducible draw receipts; server-only entropy does not prove non-selection.",
        host: "storefront", path: "/api/bounty/vault", methods: ["GET"],
        modalities: ["json"], auth: "user", provenance: "live",
        cosmology_axes: ["value", "substrate"], since: "2026-04-20" },
      { id: "storefront.leaderboards", description: "Pause-state endpoint for market rankings. It publishes no human rankings or card aggregates derived from completed trades. Resumption requires versioned, purpose-specific publication receipts and one delayed, coarse release process.",
        host: "storefront", path: "/api/leaderboards", methods: ["GET"],
        modalities: ["json"], auth: "public", provenance: "static",
        cosmology_axes: ["identity", "value", "transaction"], methodology_url: "/methodology/market",
        since: "2026-05-11" },
    ...
    verify: [
      { id: "storefront.verify.chain", description: "Hash-linked Merkle batches over revealed shared-draw receipts; rewrite detection depends on an externally retained tip.",
        host: "storefront", path: "/api/verify/chain", methods: ["GET"],
        modalities: ["json"], auth: "public", provenance: "live",
        cosmology_axes: [], methodology_url: "/methodology/fraud-flag",
        since: "2026-04-15" },
      { id: "storefront.verify.fairness", description: "Thresholded observed distributions for shared weighted draws. Low-volume exact counts are withheld and internal reward keys are replaced with response-local labels; distribution fit does not prove unbiased seed selection.",
        host: "storefront", path: "/api/verify/fairness", methods: ["GET"],
        modalities: ["json"], auth: "public", provenance: "live",
        cosmology_axes: [], since: "2026-04-15" },
      { id: "storefront.verify.health", description: "Aggregate operational status for draw digests, receipt-consistency self-audits, and distribution alerts. It omits draw ids and raw alert summaries.",
      ...
      { id: "storefront.verify.compute", description: "Pure POST computation over caller-supplied commitment, seeds, nonce, weights, and claimed rarity. It checks internal math consistency without reading a stored draw or proving unbiased input selection; OPTIONS supports CORS preflight.",
        host: "storefront", path: "/api/verify/compute", methods: ["POST", "OPTIONS"],
        modalities: ["json"], auth: "public", provenance: "computed",
    ...
    methodology: [
      ...
      { id: "storefront.sold_comps.json", description: "Paused policy surface with aggregate rights NOASSERTION and source rights internal-only. It performs no transaction-database read and publishes no transaction prices, counts, dates, conditions, or threshold totals. The former K=5 projection did not require distinct people, exposed reconstructive statistics, and lacked a purpose-specific transaction-data publication receipt; its CC0 claim was therefore withdrawn before this privacy release.",
        ...
        since: "2026-07-11" },
      { id: "storefront.sold_comps.sku", description: "Per-SKU face of the paused sold-comps policy. Aggregate rights are NOASSERTION and source rights are internal-only. It performs no transaction-database read and returns only the requested SKU, pause reasons, and an empty bucket list.",
      ...
      { id: "wholesale.ingest_quarantine.list", description: "Bearer-gated failed-normalization metadata from the data-ingest pipeline. The list returns rejection and resolution metadata plus raw-payload field names and byte size; it never returns raw_payload itself. ?source / ?unresolved / ?reason_contains / ?window. The former detail door is closed pending a separate operator-only authorization surface, so raw payloads cannot be fetched through the wholesale API. kingdom-081 Phase 4.2.",
        ...
        since: "2026-05-14" },
      { id: "wholesale.ingest_quarantine.detail", description: "Status-only boundary for quarantine detail and review mutations. GET and PATCH both return HTTP 503 before authentication or database access, with no raw payload or row metadata. A separate operator-only authorization surface must exist before either operation can open. kingdom-081 Phase 4.2b, blocked 2026-07-12.",
      ...
      { id: "storefront.catalog.jsonl", description: "Public status surface for the paused bulk catalog. Returns HTTP 503 NDJSON with a manifest and footer, zero card rows, publication_status=paused_pending_field_level_rights, and NOASSERTION. Performs no catalog database read. Reopening requires field-level upstream lineage and a reviewed bulk-publication rule. kingdom-081 Phase 5.1, paused 2026-07-12.",
        ...
        since: "2026-05-14" },
      { id: "wholesale.prices.sources", description: "Authenticated source-publication status for one card. No price source is currently cleared: returns HTTP 503 with zero rows and performs no archive read. Stored rows, authentication, and a downstream contract do not grant publication rights.",
        ...
        since: "2026-05-14" },
      { id: "wholesale.tcgplayer.history", description: "Dormant TCGplayer history route. Cambridge has no recorded approval or observations; TCGplayer is not granting new API access and its terms prohibit Cambridge's multi-source comparison use without written consent. Must remain unavailable until that approval is recorded.",
        ...
        since: "2026-05-13" },
      { id: "wholesale.tcgplayer.resolve", description: "Blocked TCGplayer identifier-resolution status door. Stored upstream identifiers are not Cambridge-owned content and are not served without written approval covering publication.",
        ...
        since: "2026-05-13" },
      { id: "storefront.cards.tcgplayer_history", description: "Blocked TCGplayer history door retained for explicit status. No credentials, approval, or observations are recorded; it must not expose TCGplayer data unless written approval explicitly covers Cambridge's multi-source use.",
      ...
      { id: "storefront.cards.everything", description: "The composer — given a canonical SKU, returns mixed card metadata and structural siblings plus explicit source-publication status. Uncleared current prices, CardRush history, legacy images, and reference values are withheld; the history array is empty and reference price is null. Aggregate card-metadata rights are NOASSERTION.",
        ...
        since: "2026-05-14" },
      { id: "storefront.cards.batch", description: "Bounded multi-card identity resolver for callers that already know which cards they need. POST 1–100 SKU strings and receive one ordered result per entry, including duplicates: found, invalid_sku, not_in_storefront_mirror, or ambiguous_mirror_match. Absence means only that the local storefront mirror had no match. Price observations, image URLs, stock, offers, and personal data stay out. The mixed mirror response is NOASSERTION; Cambridge's batch structure and SKU normalization are CC0 separately. There is no server-driven wildcard or cursor listing.",
        host: "storefront", path: "/api/v1/cards/batch", methods: ["POST"],
        modalities: ["json"], auth: "public", provenance: "live",
        cosmology_axes: ["identity", "knowledge", "substrate"], methodology_url: "docs/connections/the-card-batch.md",
        since: "2026-07-13" },
      { id: "storefront.cards.evidence", description: "Exact-SKU evidence map separating computed-reference status, live collector offers, paused completed-sale publication, paused collector-observation publication, and upstream source states. The whole mixed view is NOASSERTION. Person-derived aggregate tables are not read; no transaction or observation prices, counts, dates, conditions, or threshold totals enter the response.",
        host: "storefront", path: "/api/v1/cards/[sku]/evidence", methods: ["GET"],
        modalities: ["json"], auth: "public", provenance: "computed",
        cosmology_axes: ["identity", "value", "time", "knowledge", "authority"], methodology_url: "docs/connections/the-collector-witnesses.md",
        since: "2026-07-12" },
      { id: "storefront.card.reference", description: "Exact-SKU collector card page. Its Evidence section keeps reference status, offers, paused completed-sale publication, paused collector-observation publication, and source rights in separate visible lanes; the private Collector Witness notebook provides signed-in correction and deletion paths.",
        host: "storefront", path: "/product/[sku]", methods: ["GET"],
        modalities: ["html"], auth: "public", provenance: "computed",
        cosmology_axes: ["identity", "value", "time", "knowledge", "authority"], methodology_url: "docs/connections/the-collector-witnesses.md",
        since: "2026-07-12" },
      { id: "storefront.search.everything", description: "Convenience — resolver + composer in one round-trip. When the input resolves to a single (set, number) bucket, folds the composer payload into data.everything so a caller gets POOF in one fetch. When ambiguous, returns matches only and lets the caller disambiguate. The HTML face is /prices/search. kingdom-090.",
        ...
        since: "2026-05-14" },
      { id: "storefront.prices.search", description: "HTML search face — input card number + game to resolve mixed card identity and language variants. Source status is explicit; uncleared price magnitudes and transaction histories are withheld. Server-rendered and URL-driven (shareable permalink: /prices/search?game=op&q=OP01-001).",
        host: "storefront", path: "/prices/search", methods: ["GET"],
        modalities: ["html"], auth: "public", provenance: "live",
        cosmology_axes: ["identity", "value"], methodology_url: "docs/connections/the-bright-data-unlock.md",
        since: "2026-05-14" },
      { id: "storefront.federation.by_upstream", description: "Reserved reverse-lookup contract. TCGplayer requests return an explicit blocked status; no upstream identifier mappings are currently enabled for public publication.",
      ...
      { id: "storefront.sources.welcome", description: "The hospitality endpoint. Where /api/v1/sources is the spec sheet, this is the welcome sheet — the platform's prose welcome to each upstream river plus seven commitments. Known lineage and source tiers propagate; incomplete field-level lineage is named as NOASSERTION rather than attributed by guess. The other commitments cover rate limits, caller identification, provenance, visible failures, and arrival truth. Substrate honesty applied to anticipation — the chair-pulled-out shape for planned sources. This fixed platform-authored document is CC0; linked source data does not inherit that license. kingdom-080 (the-welcome-table.md).",
        ...
        since: "2026-05-13" },
      { id: "storefront.federation.identify_at", description: "Date-shaped compatibility surface for the current structural hash. It walks up to 5000 current catalog rows with price and capture-date inputs fixed to null; the requested date does not affect the hash and does not reconstruct historical prices or historical structural fields. Aggregate mixed-catalog rights are NOASSERTION; the Cambridge-authored resolver structure remains CC0 separately.",
        ...
        since: "2026-05-14" },
      { id: "storefront.cardrush.history", description: "Policy-status door for withheld CardRush history. Anonymous callers receive 401; signed-in callers receive HTTP 503, policy details, and an empty observations array. It performs no wholesale or archive read because authentication does not create upstream permission.",
        ...
        since: "2026-05-14" },
      { id: "storefront.webhooks.subscriptions", description: "Webhook subscription management. Auth-gated. **Design-shipped, delivery-runtime pending.** Participants can POST target_url + event_types + label; those participant-supplied and operational fields remain NOASSERTION unless the participant explicitly supplies a license. Delivery is not active.",
      ...
      { id: "storefront.rate_limits", description: "Declared rate-limit policy. Advisory; per-source freshness budgets are the polite-poll cadence. Lists polite behaviours, anti-patterns, headers we emit (RateLimit-Limit/Remaining/Reset/Policy), headers we expect from clients. kingdom-082.",
        ...
        since: "2026-05-14" },
      { id: "storefront.fx_rates", description: "Display-currency rate table for the price guide. Six ISO 4217 currencies (GBP base + USD/EUR/JPY/HKD/CHF). ECB daily EUR-reference statistics are transformed to GBP base as target_per_EUR / GBP_per_EUR, cached 6h, and served with `Source: ECB statistics`, its reuse-policy URL, source date, retrieval time, proprietary source tier, and aggregate NOASSERTION. A dated static fallback is marked is_fallback=true when ECB is unavailable. Every transaction clears in GBP.",
      ...
      { id: "storefront.examples.singleton", description: "One endpoint's canonical example with annotated_fields, when_to_use, gotchas, see_also. kingdom-083.",
        ...
        cosmology_axes: ["substrate"], methodology_url: "docs/connections/the-inner-peace.md",
        since: "2026-05-14" },
      { id: "storefront.adopters.json", description: "Public registry of participant-submitted claims about using Cambridge TCG standards. Entries are self-declarations, not Cambridge verification, and remain NOASSERTION unless the participant explicitly supplies a license. Empty today. kingdom-083.",
        host: "storefront", path: "/api/v1/adopters", methods: ["GET"],
        modalities: ["json"], auth: "public", provenance: "static",
        cosmology_axes: ["identity"], methodology_url: "/standards/adopters",
        since: "2026-05-14" },
      { id: "storefront.directory.organisations.json", description: "Public collectives with a separate directory receipt. Separates steward-written fields from platform record timestamps; selects no record ids, structured membership, platform-derived steward identity, rankings, private-row existence or inferred relationships. Free text may mention people.",
      ...
      { id: "storefront.coverage.history", description: "Bounded daily history of stored observation depth for 7, 30, or 90 UTC dates. Returns one zero-filled row per day, exact whole-window distinct-card unions, completed-day ratios that exclude the partial current UTC day, and contributing source rights; no upstream price or card-level catalog field beyond game identifiers, URL, person, or inferred relationship. Snapshot dates are archive labels rather than fetch timestamps, backfills may revise old points, and daily distinct counts are non-additive. The envelope names the CC0 Cambridge aggregation, proprietary internal catalog game mapping, and reviewed upstream tiers; unknown upstream ids default to proprietary and aggregate rights remain NOASSERTION. ?window / ?source / ?game filters. kingdom-107.",
        ...
        cosmology_axes: ["time", "substrate"], methodology_url: "docs/connections/the-aggregator-presents.md",
        since: "2026-07-13" },
      { id: "storefront.coverage_hunt.board", description: "Public read-only Coverage Hunt board derived from the declared-vs-observed coverage matrix. Candidates contain operational identifiers, counts, dates, and freshness only; no price, collector row, identity, or raw upstream content. The mixed response is NOASSERTION: CC0 covers only Cambridge's board shape and explanations, while the proprietary game mapping and upstream material retain their own rights. Blocked/planned paths can become documentation-review candidates but never acquisition tasks. Board reads create nothing; play continues through four bearer-key MCP tools.",
        host: "storefront", path: "/api/v1/coverage/hunt", methods: ["GET"],
        modalities: ["json"], auth: "public", provenance: "computed",
        cosmology_axes: ["time", "substrate", "knowledge", "authority"], methodology_url: "docs/connections/the-coverage-hunt.md",
        since: "2026-07-12" },
