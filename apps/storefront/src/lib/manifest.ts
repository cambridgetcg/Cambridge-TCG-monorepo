  ...
  resources: {
    ...
    methodology: [
      ...
      { id: "storefront.cards.tcgplayer_history", description: "Auth-gated TCGplayer USD observation history. Mirror of sister's cardrush-history (kingdom-081 5.4) for partner-redistributable tier. Per-session, 365-row cap, license_notice block echoed in response body for SDK consumption. kingdom-080.",
        ...
        cosmology_axes: ["time", "value"], methodology_url: "docs/connections/the-tcgplayer-alignment.md",
        since: "2026-05-13" },
      { id: "storefront.search.cards", description: "Card-number resolver — turn (game, query) into one or more canonical SKU candidates with confidence labels. Input shapes: 'OP01-001' (set+number, any separator style — spaces/slashes/full-width fold), '001' (number alone; fuzzy), 'op-op01-001-ja' (full canonical SKU), or a card name ('luffy', 'ルフィ'; typo-tolerant). q is 2–100 chars; ?offset paginates. Returns matches array (each row carries identity PLUS the platform's own quote: price_gbp, in_stock, rarity, set_name) + summary { count, best_confidence, distinct_set_number_buckets, ambiguous, upstream_total, truncated } + resolved_game + upstream health. Pure-compute over wholesale `cards` table. CC0. kingdom-090; search overhaul 2026-06-12.",
      ...
      { id: "storefront.cards.everything", description: "The composer — given a canonical SKU, returns everything the platform knows about that card: card meta, today's prices across every source (with agreement stats), history-summary per source (Phase 1: sparkline stats only; raw tape gated to auth tier-2), siblings across languages/variants, and the platform's own quote. Mixed license — per-source license tier declared in _meta.source_license; pokemon rows declare _meta.upstream_proxy for kingdom-088 Bright Data routing. kingdom-090.",
        ...
        since: "2026-05-14" },
      { id: "storefront.search.everything", description: "Convenience — resolver + composer in one round-trip (in-process; no self-HTTP hop). When the input resolves to a single exact (set, number) bucket, folds the composer payload into data.everything — the print is RANKED (requested lang > base print > in stock > priced) and data.fold_reason says which rule won. When ambiguous, returns matches only. q is 2–100 chars; ?lang prefers a print language; ?offset paginates fuzzy lists. data.resolved_game + data.upstream make unknown-game and upstream-outage distinguishable from empty. The HTML face is /prices/search. kingdom-090; search overhaul 2026-06-12.",
