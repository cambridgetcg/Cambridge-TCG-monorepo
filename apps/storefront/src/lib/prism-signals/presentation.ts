...
/**
 * Public presentation vocabulary for the PRISM Signals preview.
 *
 * The one synthetic signal below is an exact, strictly parsed public
 * OpportunitySignalV1. It contains no input evidence, source row, exact
 * economics, provider request, score, threshold, or model/debug field. Both
 * the web card and Telegram demo project from this same frozen value.
 */
...
const PUBLIC_ORIGIN = "https://cambridgetcg.com" as const;

function publicLink<const Path extends string>(path: Path) {
  return Object.freeze({ path, url: `${PUBLIC_ORIGIN}${path}` as const });
}

/** One source for offer paths, page links, and Telegram absolute links. */
export const PRISM_SIGNALS_LINKS = Object.freeze({
  product: publicLink("/prism-signals"),
  terms: publicLink("/prism-signals/terms"),
  support: publicLink("/contact"),
  privacy: publicLink("/privacy#prism-signals-telegram"),
  methodology: publicLink("/methodology/prism-signals"),
  signalMethodology: publicLink("/methodology/opportunity-signals"),
  offer: publicLink("/api/prism-signals/offer"),
});
...
/**
 * The canonical fixture is deliberately authored at the public output seam,
 * then validated and deeply frozen by the opportunity-signal package.
 */
    ...
    bands: Object.freeze([
      Object.freeze({
        ...
        value:
          estimate === null
            ? "Unavailable"
            : SPREAD_LABELS[estimate.conservative_net_transaction_spread_band],
