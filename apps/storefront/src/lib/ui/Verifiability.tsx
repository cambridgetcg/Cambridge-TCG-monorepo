...
export function Verifiability({ source, id, href, cite }: VerifiabilityProps) {
  if (cite) {
    return (
      <span className="inline-flex items-center gap-1 text-[11px] text-ink-muted">
        ...
        <code className="font-mono select-all text-ink-muted">{id}</code>
      </span>
    );
  }
  ...
  const inner = (
    <>
      <span className="text-ink-faint uppercase tracking-wider">{source}</span>
      <code className="ml-1 font-mono text-ink-muted" title={id}>
        {truncated}
      </code>
      {href && <span className="ml-0.5 text-ink-faint">↗</span>}
    </>
  );
  ...
  if (href) {
    return (
      <a
        ...
        className="inline-flex items-center text-[10px] hover:text-accent-strong"
