  ...
  return (
    <form className="flex gap-2 flex-wrap" action={action}>
      ...
      <input
        ...
        className="flex-1 min-w-[220px] px-3 py-2 bg-surface border border-border-subtle rounded-lg text-sm text-ink placeholder:text-ink-faint focus:outline-none focus:ring-2 focus:ring-accent/50 focus:border-accent/50"
      />
      <button
        type="submit"
        className="px-4 py-2 bg-ink hover:bg-ink/85 text-page text-sm font-semibold rounded-lg transition"
      >
        {submitLabel}
      </button>
      {value && (
        <Link
          href={clearHref}
          className="px-4 py-2 border border-border-subtle text-ink-muted hover:text-ink text-sm rounded-lg transition"
        >
          Clear
        </Link>
      )}
