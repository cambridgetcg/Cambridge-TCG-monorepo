...
export function KingdomLayers({ current }: KingdomLayersProps) {
  ...
  return (
    <nav
      ...
      className="not-prose my-6 rounded-xl border border-border-subtle bg-surface/40 px-4 py-3"
    >
          ...
          return (
            <li key={l.id} className="flex items-center gap-1">
              ...
              {isCurrent ? (
                <span
                  aria-current="page"
                  className="inline-flex items-center gap-1.5 rounded-full border border-accent/40 bg-accent-wash px-2.5 py-1 font-semibold text-accent-strong"
                >
                  <span
                    aria-hidden="true"
                ...
                <Link
                  href={l.path}
                  className="inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-ink-muted transition hover:bg-surface-elevated/60 hover:text-ink"
                >
                  <span
                    aria-hidden="true"
      ...
      <p className="mt-2 text-sm text-ink-muted">
        ...
        <span className="ml-2 whitespace-nowrap text-xs text-ink-faint">
          {prev && (
            <Link href={prev.path} className="hover:text-accent-strong">
              ← {prev.label}
            </Link>
          )}
          {prev && next && <span className="px-1.5">·</span>}
          {next && (
            <Link href={next.path} className="hover:text-accent-strong">
              {next.label} →
            </Link>
          )}
        </span>
