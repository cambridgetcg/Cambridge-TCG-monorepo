...
export function TypeSignature({
  ...
  return (
    <aside
      ...
      className="not-prose mt-12 rounded-lg border border-border-subtle bg-surface-subtle p-4 text-xs"
    ...
    >
      ...
      <dl className="space-y-1.5 text-ink-muted">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <dd className="flex-1 min-w-0">
            {doctrines.map((d, i) => (
              <span key={d}>
                <span className="text-ok">{DOCTRINE_LABELS[d]}</span>
                {i < doctrines.length - 1 && <span className="text-ink-faint">, </span>}
              </span>
            ))}
          </dd>
        </div>
            ...
            <dd className="flex-1 min-w-0">
              {recursion.map((r, i) => (
                <span key={r.href}>
                  ...
                  {i < recursion.length - 1 && <span className="text-ink-faint">, </span>}
                </span>
              ))}
            </dd>
          </div>
      ...
      <footer className="mt-3 pt-2 border-t border-border-subtle text-[10px] text-ink-faint">
