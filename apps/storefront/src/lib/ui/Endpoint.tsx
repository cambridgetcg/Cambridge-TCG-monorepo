...
export function Endpoint({ path, method = "GET", format = "JSON" }: EndpointProps) {
  return (
    <a
      href={path}
      className="not-prose inline-flex max-w-full items-center gap-1.5 rounded-md border border-border-strong bg-surface px-2 py-0.5 align-middle font-mono text-xs text-ink-muted transition hover:border-neutral-500 hover:text-ink"
    >
      <span aria-hidden="true" className="text-ink-faint">
        {"{ }"}
      </span>
      {method !== "GET" && (
        <span className="font-semibold text-accent-strong">{method}</span>
      )}
      <span className="truncate">{path}</span>
      <span className="rounded-sm bg-surface-elevated px-1 text-[10px] uppercase tracking-wide text-ink-muted">
        {format}
      </span>
    </a>
  );
...
// Chip-named alias — same component.
export { Endpoint as EndpointChip };
