...
export const PLAY_RESOURCES: readonly PlayResource[] = [
  ...
  },
  {
    ...
    layer: "L2_pure_fn",
    status: "shipped",
    blurb: "Client-side deck helpers shared by hub/adventure/multiplayer — SavedDeck localStorage shape, deck→card-list conversion, and the auto-mounted ST-15 default so a deckless visitor never hits a wall.",
    composes_with: ["lib_starter_decks"],
    serves_archetypes: ["hobbyist"],
  },
  {
    ...
    layer: "L3_runtime",
    status: "shipped",
    blurb: "Server-side starter-deck resolution — card_number refs joined to the wholesale catalog (SKU, name, image, rarity); single source for the starters read view and the PvE/PvP load payload.",
    composes_with: ["lib_starter_decks", "api_starter_deck"],
    serves_archetypes: ["hobbyist"],
  },
  {
