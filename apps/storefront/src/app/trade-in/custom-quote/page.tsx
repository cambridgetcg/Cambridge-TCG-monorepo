...
const inputClass =
  "w-full px-4 py-3 bg-surface border border-border-subtle rounded-lg text-ink placeholder:text-ink-faint focus:outline-none focus:ring-2 focus:ring-accent/50";
      ...
      <div className="max-w-2xl mx-auto px-4 py-8">
        ...
        <div className="space-y-6 mb-8">
          {cards.map((card, index) => (
            <div
              key={card.id}
              className="bg-surface border border-border-subtle rounded-lg p-5 relative"
            >
              {/* Card number badge */}
              <div className="flex items-center justify-between mb-4">
                <span className="inline-flex items-center justify-center w-7 h-7 rounded-full bg-ink text-page text-xs font-bold">
                  #{index + 1}
                </span>
                {cards.length > 1 && (
                  <button
                    ...
                    className="w-7 h-7 flex items-center justify-center rounded-full text-ink-faint hover:text-danger hover:bg-surface-subtle transition"
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  {card.images.length > 0 && (
                    <div className="flex flex-wrap gap-2 mb-3">
                      {card.images.map((img, imgIdx) => (
                        <div
                          key={imgIdx}
                          className="relative w-16 h-16 rounded-lg overflow-hidden bg-surface-subtle group"
                        >
                          ...
                          <button
                            ...
                            className="absolute inset-0 bg-ink/60 opacity-0 group-hover:opacity-100 flex items-center justify-center transition"
                  ...
                  <button
                    ...
                    className="px-4 py-2 text-sm border border-border-subtle text-ink-muted rounded-lg hover:border-border-strong hover:text-ink transition disabled:opacity-50 disabled:cursor-not-allowed"
                  >
        ...
        <button
          type="button"
          onClick={addCard}
          className="w-full py-3 border-2 border-dashed border-border-subtle text-ink-muted rounded-lg hover:border-accent/50 hover:text-accent-strong transition font-medium mb-10"
        >
          + Add Another Card
        </button>
        ...
        {/* Error */}
        {error && (
          <p className="text-sm text-danger bg-danger/10 rounded-lg px-4 py-3 mb-6">
            {error}
          </p>
        )}
