    ...
    return (
      <main className="min-h-screen bg-page">
        <div className="max-w-2xl mx-auto px-4 py-20 text-center">
          <h1 className="text-2xl font-bold text-ink mb-4">No Cards Selected</h1>
          <p className="text-ink-muted mb-6">Add cards from the buylist first.</p>
          <Link
            href="/trade-in"
    ...
    <main className="min-h-screen bg-page">
      <div className="max-w-2xl mx-auto px-4 py-8">
        ...
        <div className="flex items-center gap-3 mb-8">
          <div className={`flex items-center gap-2 ${step >= 1 ? "text-accent" : "text-ink-faint"}`}>
          ...
          <div className={`flex items-center gap-2 ${step >= 2 ? "text-accent" : "text-ink-faint"}`}>
        ...
        {step === 1 && (
          <div>
            ...
            <div className="space-y-3 mb-6">
              {items.map((item) => (
                <div key={item.sku} className="flex gap-3 bg-surface border border-border-subtle rounded-lg p-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium truncate text-ink">{item.name}</p>
                    <p className="text-xs text-ink-faint">{item.card_number} · {gameLabel(item.game)}</p>
                    <div className="flex items-center gap-2 mt-2">
                      <button onClick={() => updateQty(item.sku, item.quantity - 1)} className="w-9 h-9 bg-surface-subtle hover:bg-surface-subtle rounded text-sm font-bold transition">-</button>
                  ...
                  <div className="text-right shrink-0">
                    <p className="text-xs text-ink-faint">Cash</p>
                    <p className="text-sm text-accent">{formatPrice(item.cash_price * item.quantity)}</p>
                    <p className="text-xs text-ink-faint mt-1">Credit</p>
                    <p className="text-sm text-accent">{formatPrice(item.credit_price * item.quantity)}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="bg-surface border border-border-subtle rounded-lg p-4 mb-6">
              <h3 className="text-sm font-bold text-ink mb-3">Payment Method</h3>
              <div className="flex flex-col sm:flex-row gap-3">
                <label className={`flex-1 cursor-pointer rounded-lg border-2 p-3 text-center transition ${paymentMethod === "credit" ? "border-accent bg-accent-wash" : "border-border-subtle hover:border-border-strong"}`}>
                  ...
                  <p className="text-lg font-bold text-accent mt-1">{formatPrice(creditTotal)}</p>
                  <p className="text-xs text-ink-muted mt-1">1 business day</p>
                </label>
                <label className={`flex-1 cursor-pointer rounded-lg border-2 p-3 text-center transition ${paymentMethod === "cash" ? "border-accent bg-accent-wash" : "border-border-subtle hover:border-border-strong"}`}>
                  ...
                  <p className="text-lg font-bold text-accent mt-1">{formatPrice(cashTotal)}</p>
                  <p className="text-xs text-ink-muted mt-1">2 business days</p>
                </label>
              </div>
            </div>
            ...
            <div className="bg-surface border border-border-subtle rounded-lg p-4 mb-6">
              <div className="flex justify-between text-lg font-bold">
                <span className="text-ink">Total ({paymentMethod === "cash" ? "Cash" : "Credit"})</span>
            ...
            <button
              onClick={() => setStep(2)}
              className="w-full py-3 bg-ink text-page font-bold rounded-lg hover:opacity-90 transition"
            >
              Continue to Details
            </button>
          </div>
        ...
        {step === 2 && (
          <div className="space-y-6">
            ...
            {paymentMethod === "cash" && (
              <div className="space-y-4">
                <h3 className="text-sm font-bold text-ink">Bank Details</h3>
                {!provideBankLater && (
                  <>
                    <input
                      type="text"
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  />
                  <span className="text-sm text-ink-muted">I&apos;ll provide bank details later</span>
                </label>
              </div>
            )}
            ...
            <div className="space-y-3">
              <h3 className="text-sm font-bold text-ink">Delivery Method</h3>
              <div className="flex flex-col sm:flex-row gap-3">
                <label className={`flex-1 cursor-pointer rounded-lg border-2 p-3 transition ${deliveryMethod === "mail" ? "border-accent bg-accent-wash" : "border-border-subtle hover:border-border-strong"}`}>
                  ...
                  <p className="text-xs text-ink-muted mt-1">Post your cards to us</p>
                </label>
                <label className={`flex-1 cursor-pointer rounded-lg border-2 p-3 transition ${deliveryMethod === "instore" ? "border-accent bg-accent-wash" : "border-border-subtle hover:border-border-strong"}`}>
                  ...
                  <p className="text-xs text-ink-muted mt-1">Drop off in person</p>
                </label>
              </div>
            </div>
            ...
            {/* Notes */}
            <div>
              <h3 className="text-sm font-bold text-ink mb-2">Notes (optional)</h3>
              <textarea
            ...
            {/* Declarations */}
            <div className="space-y-3 bg-surface border border-border-subtle rounded-lg p-4">
            ...
            <div className="bg-surface border border-border-subtle rounded-lg p-4">
              <div className="flex justify-between text-lg font-bold">
                <span className="text-ink">{paymentMethod === "cash" ? "Cash" : "Credit"} Payout</span>
                <span className="text-accent">{formatPrice(total)}</span>
              </div>
            ...
            {error && (
              <p className="text-sm text-danger bg-danger/10 rounded-lg px-4 py-3">
                {error}
              </p>
            )}
            ...
            <p className="text-xs text-ink-faint text-center">
              ...
              <Link href="/trade-in/terms" className="text-accent hover:underline">
                trade-in terms
              </Link>
              .
            </p>
