    ...
    <main className="min-h-screen bg-page">
      <div className="max-w-2xl mx-auto px-4 py-8">
        ...
        <div className="flex items-center gap-3 mb-8">
          <div className={`flex items-center gap-2 ${step >= 1 ? "text-accent-strong" : "text-neutral-600"}`}>
          ...
          <div className={`flex items-center gap-2 ${step >= 2 ? "text-accent-strong" : "text-neutral-600"}`}>
        ...
        {step === 1 && (
          <div>
            ...
            <div className="space-y-3 mb-6">
              {items.map((item) => (
                <div key={item.sku} className="flex gap-3 bg-surface rounded-xl p-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      ...
                      <button onClick={() => updateQty(item.sku, item.quantity + 1)} className="w-9 h-9 bg-neutral-700 hover:bg-neutral-600 rounded text-sm font-bold transition">+</button>
                      <button onClick={() => removeItem(item.sku)} className="ml-auto text-xs text-ink-faint hover:text-red-400 transition">Remove</button>
                    </div>
                  </div>
                  <div className="text-right shrink-0">
                    <p className="text-xs text-ink-faint">Cash</p>
                    <p className="text-sm text-accent-strong">{formatPrice(item.cash_price * item.quantity)}</p>
                    <p className="text-xs text-ink-faint mt-1">Credit</p>
                    <p className="text-sm text-accent-strong">{formatPrice(item.credit_price * item.quantity)}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="bg-surface rounded-xl p-4 mb-6">
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                <label className={`flex-1 cursor-pointer rounded-lg border-2 p-3 text-center transition ${paymentMethod === "credit" ? "border-accent bg-accent/10" : "border-border-strong hover:border-neutral-600"}`}>
                  ...
                  <p className="text-lg font-bold text-accent-strong mt-1">{formatPrice(creditTotal)}</p>
                  <p className="text-xs text-ink-muted mt-1">1 business day</p>
                </label>
                <label className={`flex-1 cursor-pointer rounded-lg border-2 p-3 text-center transition ${paymentMethod === "cash" ? "border-accent bg-accent/10" : "border-border-strong hover:border-neutral-600"}`}>
                  ...
                  <p className="text-lg font-bold text-accent-strong mt-1">{formatPrice(cashTotal)}</p>
            ...
            <div className="bg-surface rounded-xl p-4 mb-6">
              <div className="flex justify-between text-lg font-bold">
                ...
                <span className="text-accent-strong">{formatPrice(total)}</span>
              </div>
        ...
        {step === 2 && (
          <div className="space-y-6">
            ...
            <div className="space-y-3">
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                <label className={`flex-1 cursor-pointer rounded-lg border-2 p-3 transition ${deliveryMethod === "mail" ? "border-accent bg-accent/10" : "border-border-strong hover:border-neutral-600"}`}>
                ...
                <label className={`flex-1 cursor-pointer rounded-lg border-2 p-3 transition ${deliveryMethod === "instore" ? "border-accent bg-accent/10" : "border-border-strong hover:border-neutral-600"}`}>
            ...
            {error && (
              <p className="text-sm text-red-400 bg-danger/10 rounded-lg px-4 py-3">
                {error}
              </p>
            )}
            ...
            <p className="text-xs text-ink-faint text-center">
              ...
              <Link href="/trade-in/terms" className="text-accent-strong hover:underline">
                trade-in terms
              </Link>
              .
            </p>
