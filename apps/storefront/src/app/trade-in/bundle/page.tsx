...
export default function BundleTradeInPage() {
  ...
  const inputClass = "w-full px-4 py-3 bg-surface border border-border-subtle rounded-lg text-ink placeholder:text-ink-faint focus:outline-none focus:ring-2 focus:ring-accent/50";
        ...
        <div className="bg-surface border border-border-subtle rounded-lg p-5 mb-8">
          ...
          <div className="grid gap-3 sm:grid-cols-3 text-sm">
            <div className="flex gap-3">
              <span className="shrink-0 w-6 h-6 bg-accent-wash text-accent rounded-full flex items-center justify-center text-xs font-bold">1</span>
              <p className="text-ink-muted">Upload photos of your cards — front, back, overview shots</p>
            </div>
            <div className="flex gap-3">
              <span className="shrink-0 w-6 h-6 bg-accent-wash text-accent rounded-full flex items-center justify-center text-xs font-bold">2</span>
              <p className="text-ink-muted">We review and send you an offer within 1-2 business days</p>
            </div>
            <div className="flex gap-3">
              <span className="shrink-0 w-6 h-6 bg-accent-wash text-accent rounded-full flex items-center justify-center text-xs font-bold">3</span>
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            {images.length > 0 && (
              <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-2 mb-3">
                {images.map((img, i) => (
                  <div key={i} className="relative aspect-square rounded-lg overflow-hidden bg-surface-subtle group">
                    <Image src={img.url} alt={`Photo ${i + 1}`} fill className="object-cover" sizes="100px" />
                    <button
                      onClick={() => removeImage(i)}
                      className="absolute top-1 right-1 w-5 h-5 bg-danger text-page rounded-full text-xs opacity-0 group-hover:opacity-100 transition flex items-center justify-center"
                    >
                      &times;
                    </button>
                  </div>
            ...
            <button
              ...
              className="w-full py-3 border-2 border-dashed border-border-subtle rounded-lg text-ink-muted hover:border-accent/50 hover:text-ink transition disabled:opacity-50"
            >
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div>
              ...
              <div className="flex gap-3">
                <label className={`flex-1 cursor-pointer rounded-lg border-2 p-3 text-center transition ${deliveryMethod === "mail" ? "border-accent bg-accent-wash" : "border-border-subtle hover:border-border-strong"}`}>
                ...
                <label className={`flex-1 cursor-pointer rounded-lg border-2 p-3 text-center transition ${deliveryMethod === "instore" ? "border-accent bg-accent-wash" : "border-border-subtle hover:border-border-strong"}`}>
          ...
          <p className="text-xs text-ink-faint text-center">
            ...
            <Link href="/trade-in/terms" className="text-accent hover:underline">trade-in terms</Link>.
          </p>
        </div>
      </div>
    </main>
