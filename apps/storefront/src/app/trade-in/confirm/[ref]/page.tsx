...
export default function ConfirmPage() {
    ...
    return (
      <main className="min-h-screen bg-page">
        <div className="max-w-2xl mx-auto px-4 py-20 text-center">
          ...
          <Link href="/trade-in" className="px-6 py-3 bg-accent text-black font-bold rounded-lg hover:bg-accent-strong transition">
            Back to Trade-In
          </Link>
        </div>
      </main>
      ...
      {submission.deliveryMethod === "mail" ? (
        <>
          ...
          <p className="text-xs text-ink-faint mt-3">
            Include your reference number <strong className="text-accent-strong">{submission.reference}</strong> on the package.
          </p>
        </>
      ) : (
        <>
          ...
          <p className="text-lg font-bold text-accent-strong mt-2">{submission.reference}</p>
        </>
      )}
    </div>
  );
      ...
      <main className="min-h-screen bg-page">
        <div className="max-w-2xl mx-auto px-4 py-12">
          ...
          <div className="bg-surface rounded-xl p-6 text-center mb-6">
            ...
            <p className="text-3xl font-black text-accent-strong tracking-wider">{submission.reference}</p>
            {submittedDate && (
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <Link
              href="/trade-in"
              className="flex-1 text-center px-6 py-3 bg-accent text-black font-bold rounded-lg hover:bg-accent-strong transition"
            >
              Trade More Cards
            </Link>
          </div>
        ...
        <div className="max-w-2xl mx-auto px-4 py-12">
          <div className="text-center mb-10">
            <div className="w-16 h-16 bg-accent/20 rounded-full flex items-center justify-center mx-auto mb-4">
              <svg xmlns="http://www.w3.org/2000/svg" className="w-8 h-8 text-accent-strong" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
              </svg>
            </div>
            <h1 className="text-2xl md:text-3xl font-bold text-ink">Your Quotation is Ready</h1>
            <p className="text-ink-muted mt-2">Reference: <span className="text-accent-strong font-bold">{submission.reference}</span></p>
          </div>
          ...
          {/* Quotation summary */}
          <div className="bg-surface border border-accent/30 rounded-xl p-6 mb-6">
            ...
            <div className="space-y-2 text-sm">
              ...
              {submission.mintBonusApplied && submission.mintBonusAmount > 0 && (
                <div className="flex justify-between">
                  <span className="text-secondary">MINT bonus</span>
                  <span className="text-secondary">+ {formatPrice(submission.mintBonusAmount)}</span>
                </div>
              )}
              ...
              {submission.finalTotal > 0 && (
                <div className="flex justify-between pt-2 border-t border-border-strong">
                  ...
                  <span className="text-accent-strong font-bold text-lg">{formatPrice(submission.finalTotal)}</span>
                </div>
              )}
            </div>

            {submission.adminMessage && (
              <div className="mt-4 bg-surface-elevated rounded-lg p-3">
            ...
            {!countdownExpired && (
              <div className="mt-6 flex flex-col sm:flex-row gap-3">
                ...
                <button
                  ...
                  disabled={actionLoading}
                  className="flex-1 px-6 py-3 bg-surface-elevated text-ink-muted font-medium rounded-lg hover:bg-neutral-700 transition disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  Decline
                </button>
              </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <Link
              href="/trade-in"
              className="flex-1 text-center px-6 py-3 bg-surface-elevated text-ink font-medium rounded-lg hover:bg-neutral-700 transition"
            >
              Back to Trade-In
            </Link>
          </div>
            ...
            <div className="bg-surface rounded-xl p-6 max-w-md w-full">
              ...
              <div className="flex gap-3">
                <button
                  ...
                  disabled={actionLoading}
                  className="flex-1 px-4 py-2 bg-red-600 text-ink font-bold rounded-lg hover:bg-danger transition disabled:opacity-50"
                >
                ...
                <button
                  ...
                  disabled={actionLoading}
                  className="flex-1 px-4 py-2 bg-surface-elevated text-ink-muted font-medium rounded-lg hover:bg-neutral-700 transition"
                >
                  Cancel
                </button>
              </div>
  ...
  if (["accepted", "received", "grading", "approved", "paid"].includes(submission.status)) {
    ...
    const STAGE_TONE: Record<string, { bg: string; text: string }> = {
      emerald: { bg: "bg-emerald-500/20", text: "text-secondary" },
      ...
      <main className="min-h-screen bg-page">
        <div className="max-w-2xl mx-auto px-4 py-20 text-center">
          ...
          <p className="text-ink-muted mb-2">
            You declined the quotation for <span className="text-accent-strong font-bold">{submission.reference}</span>.
          </p>
    ...
    return (
      <main className="min-h-screen bg-page">
        <div className="max-w-2xl mx-auto px-4 py-20 text-center">
          ...
          <p className="text-ink-muted mb-2">
            The quotation for <span className="text-accent-strong font-bold">{submission.reference}</span> has expired.
          </p>
    ...
    return (
      <main className="min-h-screen bg-page">
        <div className="max-w-2xl mx-auto px-4 py-20 text-center">
          ...
          <p className="text-ink-muted mb-2">
            We weren&apos;t able to accept trade-in <span className="text-accent-strong font-bold">{submission.reference}</span> after grading.
          </p>
      ...
      <main className="min-h-screen bg-page">
        <div className="max-w-2xl mx-auto px-4 py-20 text-center">
          ...
          <p className="text-ink-muted mb-8">
            Trade-in <span className="text-accent-strong font-bold">{submission.reference}</span> has been cancelled.
          </p>
          <Link
            href="/trade-in"
            className="px-6 py-3 bg-accent text-black font-bold rounded-lg hover:bg-accent-strong transition"
  ...
  return (
    <main className="min-h-screen bg-page">
      <div className="max-w-2xl mx-auto px-4 py-20 text-center">
        ...
        <p className="text-ink-muted mb-2">Reference: <span className="text-accent-strong font-bold">{submission.reference}</span></p>
