    ...
    <div className="min-h-screen bg-page text-ink">
      <div className="max-w-4xl mx-auto px-4 py-12">
        <header className="mb-10">
          ...
          <h1 className="text-3xl font-display font-semibold mb-3">Guides</h1>
          ...
          {GUIDES.map((g) => (
            <li key={g.slug}>
              <Link
                ...
                className="block p-5 bg-surface border border-border-subtle rounded-lg hover:border-accent/40 hover:bg-surface-subtle transition group"
              >
                <div className="flex items-baseline justify-between gap-4 mb-2">
                  <h2 className="text-lg font-semibold group-hover:text-accent transition">
                    {g.title}
                  </h2>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {g.audiences.map((a) => (
                    <span
                      key={a}
                      className="inline-block text-[10px] uppercase tracking-wider px-2 py-0.5 bg-surface-subtle text-ink-muted border border-border-subtle rounded"
                    >
                      {AUDIENCE_LABELS[a]}
                    </span>
                  ))}
