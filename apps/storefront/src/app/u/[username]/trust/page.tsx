...
function fmtDelta(n: number | null): { text: string; tone: string } {
  ...
  if (n > 0) return { text: `+${n}`, tone: "text-secondary" };
    ...
    <div className="min-h-screen bg-page text-ink">
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        <div className="flex items-baseline justify-between gap-4 mb-6 flex-wrap">
          <div>
            <p className="text-xs text-ink-faint mb-1">
              <Link href={`/u/${username}`} className="hover:text-accent-strong transition">
            ...
            <p className="text-sm text-ink-muted mt-1">
              ...
              {state.flags.is_suspended && (
                <span className="inline-flex items-center text-[11px] px-2 py-0.5 bg-danger/15 text-red-400 border border-danger/30 rounded ml-2">
                  Suspended
                </span>
              )}
            </p>
        ...
        <p className="text-sm text-ink-muted mb-8 max-w-2xl">
          ...
          <code className="text-ink-muted">?</code> glyph linking to its
              ...
              {state.reviews.total === 0 ? (
                ...
                <>
                  <div className="flex items-baseline gap-2 mb-3">
                    <span className="text-2xl font-mono text-accent-strong">
                  ...
                  <div className="border-t border-border-subtle pt-3 mt-3 space-y-1 text-xs">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-mono text-ink-muted">{fmtRating(state.reviews.sub_ratings_avg.card_accuracy)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-ink-muted">Shipping speed</span>
                      <span className="font-mono text-ink-muted">{fmtRating(state.reviews.sub_ratings_avg.shipping_speed)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-ink-muted">Communication</span>
                      <span className="font-mono text-ink-muted">{fmtRating(state.reviews.sub_ratings_avg.communication)}</span>
                    </div>
                  </div>
                </>
              )}
        ...
        <footer className="mt-12 pt-6 border-t border-border-subtle text-xs text-ink-faint space-y-2">
          ...
          <p>
            <Link href="/methodology/trust-score" className="text-accent-strong hover:underline">
