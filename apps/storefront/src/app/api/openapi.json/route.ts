  ...
  paths: {
    ...
    "/api/v1/coverage": {
      get: {
        ...
        description: "Operational observation counts, distinct-card counts, snapshot ranges, and freshness grouped by game and source. No upstream price value is returned. Responses separately name Cambridge's CC0 aggregation, the proprietary internal catalog game-mapping step, and actual contributing upstream sources with their reviewed rights tiers; aggregate rights remain NOASSERTION. Returns 503 when the wholesale observation database cannot answer, the per-process read ceiling is full, or the coverage role reaches its connection limit.",
        operationId: "getCoverage",
        parameters: [
          { name: "source", in: "query", required: false, schema: { type: "string", minLength: 1, maxLength: 64, pattern: "^[A-Za-z0-9][A-Za-z0-9._-]*$" } },
          { name: "game", in: "query", required: false, schema: { type: "string", minLength: 1, maxLength: 64, pattern: "^[A-Za-z0-9][A-Za-z0-9._-]*$" } },
          { name: "since", in: "query", required: false, schema: { type: "string", minLength: 10, maxLength: 10, format: "date" } },
        ],
        responses: {
          ...
          "503": { description: "Observation database unavailable, per-process read capacity full, or coverage-role connection capacity full." },
        },
      },
    },
    "/api/v1/coverage/history": {
      get: {
        tags: ["inspectability", "catalog"],
        summary: "Daily observation archive coverage history",
        description: "One zero-filled row per UTC date for a bounded 7d, 30d, or 90d window, with daily operational counts and exact whole-window distinct-card unions. Completed-day ratios exclude the current UTC day because it may still be partial. Snapshot dates are stored archive labels, not fetch timestamps; backfills may revise prior points, and daily distinct-card counts are non-additive. No upstream price or card-level catalog field beyond game identifiers, URL, person, or inferred relationship is returned. The envelope separately names Cambridge's CC0 aggregation, the proprietary internal catalog game-mapping step, and every actually observed upstream source with its reviewed rights tier; unknown upstream ids default to proprietary. Responses retain explicit aggregate NOASSERTION. Returns 503 when the observation database cannot answer, the per-process read ceiling is full, or the coverage role reaches its connection limit.",
        operationId: "getCoverageHistory",
        parameters: [
          { name: "window", in: "query", required: false, schema: { type: "string", enum: ["7d", "30d", "90d"], default: "30d" } },
          { name: "source", in: "query", required: false, schema: { type: "string", minLength: 1, maxLength: 64, pattern: "^[A-Za-z0-9][A-Za-z0-9._-]*$" } },
          { name: "game", in: "query", required: false, schema: { type: "string", minLength: 1, maxLength: 64, pattern: "^[A-Za-z0-9][A-Za-z0-9._-]*$" } },
        ],
        responses: {
          "200": { description: "Daily coverage history with per-source rights in the envelope.", content: { "application/json": { schema: { $ref: "#/components/schemas/CoverageHistoryEnvelope" } } } },
          "400": { description: "Invalid window or identifier filter." },
          "503": { description: "Observation database unavailable, per-process read capacity full, or coverage-role connection capacity full." },
        },
      },
    },
    ...
    schemas: {
      ...
      CoverageHistoryDay: {
        ...
        properties: {
          ...
          is_current_utc_day: { type: "boolean" },
          observations: { type: "integer", minimum: 0 },
          distinct_cards: { type: "integer", minimum: 0, description: "Non-additive across days." },
          distinct_games: { type: "integer", minimum: 0 },
          distinct_sources: { type: "integer", minimum: 0 },
          unassigned_observations: { type: "integer", minimum: 0 },
          sources: { type: "array", uniqueItems: true, items: { type: "string" } },
          games: { type: "array", uniqueItems: true, items: { type: "string" } },
        },
      },
      CoverageHistoryPeriod: {
        type: "object",
        additionalProperties: false,
        required: ["start", "through", "timezone", "current_utc_day_may_be_incomplete"],
        properties: {
          start: { type: "string", format: "date" },
          through: { type: "string", format: "date" },
          timezone: { type: "string", const: "UTC" },
          current_utc_day_may_be_incomplete: { type: "boolean", const: true },
        },
      },
      CoverageHistorySummary: {
        type: "object",
        additionalProperties: false,
        required: [
          "total_observations",
        ...
        properties: {
          ...
          distinct_cards: { type: "integer", minimum: 0, description: "Exact distinct-card union across the whole requested window." },
          distinct_games: { type: "integer", minimum: 0 },
          distinct_sources: { type: "integer", minimum: 0 },
          unassigned_observations: { type: "integer", minimum: 0 },
          completed_days: { type: "integer", minimum: 0, description: "Requested UTC dates excluding the possibly partial current UTC day." },
          observed_completed_days: { type: "integer", minimum: 0, description: "Completed UTC dates with at least one stored observation." },
          zero_observation_completed_days: { type: "integer", minimum: 0, description: "Completed UTC dates with no stored observation." },
          observation_completed_day_ratio: { type: "number", minimum: 0, maximum: 1, description: "observed_completed_days divided by completed_days; the current UTC day is excluded." },
          observed_days_including_current: { type: "integer", minimum: 0, description: "All requested UTC dates with at least one stored observation, including today when applicable." },
        },
      },
      CoverageHistoryFilter: {
        type: "object",
        additionalProperties: false,
        required: ["window", "window_days", "source", "game"],
        properties: {
          window: { type: "string", enum: ["7d", "30d", "90d"] },
          window_days: { type: "integer", enum: [7, 30, 90] },
          source: { type: ["string", "null"], minLength: 1, maxLength: 64, pattern: "^[A-Za-z0-9][A-Za-z0-9._-]*$" },
          game: { type: ["string", "null"], minLength: 1, maxLength: 64, pattern: "^[A-Za-z0-9][A-Za-z0-9._-]*$" },
        },
      },
      CoverageHistoryData: {
        type: "object",
        ...
        properties: {
          "@kind": { type: "string", const: "aggregator_coverage_history" },
          description: { type: "string" },
          period: { $ref: "#/components/schemas/CoverageHistoryPeriod" },
          summary: { $ref: "#/components/schemas/CoverageHistorySummary" },
          by_day: { type: "array", minItems: 7, maxItems: 90, items: { $ref: "#/components/schemas/CoverageHistoryDay" } },
          observed_sources: { type: "array", uniqueItems: true, items: { type: "string" }, description: "Actually observed upstream source ids only; Cambridge processing sources are declared in _meta.sources." },
          filter: { $ref: "#/components/schemas/CoverageHistoryFilter" },
          upstream_queried_at: { type: "string", format: "date-time" },
          measurement: {
            type: "object",
            additionalProperties: false,
            required: ["observation", "snapshot_date", "comparability", "breadth", "daily_distinct_counts", "completed_day_ratio"],
            properties: {
              observation: { type: "string" },
              snapshot_date: { type: "string" },
              comparability: { type: "string" },
              breadth: { type: "string" },
              daily_distinct_counts: { type: "string" },
              completed_day_ratio: { type: "string" },
            },
          },
          related: {
            type: "object",
            additionalProperties: false,
            required: ["current_coverage", "source_registry", "source_run_diagnosis", "platform_status"],
            properties: {
              current_coverage: { type: "string", const: "/api/v1/coverage" },
              source_registry: { type: "string", const: "/api/v1/sources" },
              source_run_diagnosis: { type: "string", const: "/api/v1/sources/[id]" },
              platform_status: { type: "string", const: "/api/v1/status" },
            },
          },
        },
      },
      CoverageHistoryEnvelope: {
        type: "object",
        required: ["data", "_meta"],
        properties: {
          data: { $ref: "#/components/schemas/CoverageHistoryData" },
          _meta: {
            allOf: [
              { $ref: "#/components/schemas/ResponseMeta" },
              {
                ...
                properties: {
                  sources: { type: "array", minItems: 2, prefixItems: [{ type: "string", const: "cambridge-tcg.coverage-aggregation" }, { type: "string", const: "cambridge-tcg.catalog-game-mapping" }], items: { type: "string" }, description: "Begins with Cambridge aggregation and catalog game mapping, followed by actually observed upstream source ids." },
                  source_license: { type: "array", minItems: 2, prefixItems: [{ type: "string", const: "cc0" }, { type: "string", const: "proprietary" }], items: { type: "string" }, description: "Parallel to sources: cc0 for Cambridge aggregation, proprietary for catalog game mapping, then the reviewed tier for each upstream id; unknown upstream ids default to proprietary." },
                  license: { type: "string", const: "NOASSERTION" },
                },
              },
            ],
          },
