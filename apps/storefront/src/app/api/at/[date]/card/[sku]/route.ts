import { NextResponse } from "next/server";

export async function GET(
  _request: Request,
  { params }: { params: Promise<{ date: string; sku: string }> },
): Promise<Response> {
  const { date, sku } = await params;
  return NextResponse.json(
    {
      "@encoding": "cambridge-tcg/universal/v1",
      "@kind": "temporal_card_publication_gap",
      record_license: "NOASSERTION",
      publication_status: "withheld-untraced-lineage",
      query: { date, sku, origin: "caller-supplied" },
      catalog_membership_asserted: false,
      historical_snapshot_asserted: false,
      resolved: false,
      does_not_include: [
        "card, set, or game membership",
        "historical observations, dates, prices, hashes, names, or images",
        "database queries",
      ],
    },
    {
      status: 503,
      headers: {
        "Cache-Control": "no-store",
        "Retry-After": "300",
        "X-Content-License": "NOASSERTION",
        "X-Schema-License": "CC0-1.0",
      },
    },
  );
}

export async function OPTIONS(): Promise<Response> {
  return new Response(null, {
    status: 204,
    headers: {
      "Access-Control-Allow-Origin": "*",
      "Access-Control-Allow-Methods": "GET, OPTIONS",
      "Access-Control-Max-Age": "86400",
    },
  });
}
