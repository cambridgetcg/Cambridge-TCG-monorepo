...
vi.mock("@/lib/release/market-payment-creation", () => ({
  getMarketPaymentCreationAvailability: vi.fn(),
}));
...
beforeEach(() => {
  vi.resetAllMocks();
  vi.mocked(getMarketPaymentCreationAvailability).mockReturnValue({
    mode: "ledger-v2-enabled",
    enabled: true,
    reason: null,
  });
...
describe("POST /api/market/trades/[id]/pay", () => {
  it("fails before reading the trade or calling Stripe while paused", async () => {
    vi.mocked(getMarketPaymentCreationAvailability).mockReturnValueOnce({
      mode: "paused",
      enabled: false,
    ...
    expect(reserveStripeCheckoutAttempt).not.toHaveBeenCalled();
  });
  ...
  it("still authenticates before revealing the operational pause", async () => {
    ...
    expect(getMarketPaymentCreationAvailability).not.toHaveBeenCalled();
    expect(query).not.toHaveBeenCalled();
    expect(getStripe).not.toHaveBeenCalled();
  });
