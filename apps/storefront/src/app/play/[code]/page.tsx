...
export default function GameBoard() {
  ...
  function PlayerArea({
    ...
    return (
      <div className={`rounded-xl p-3 sm:p-4 ${isOwn ? "bg-surface/80" : "bg-surface/40"}`}>
        {/* Label row */}
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2">
            <span className={`font-bold text-sm ${isOwn ? "text-accent-strong" : "text-ink-muted"}`}>
              {label}
            </span>
            {isOwn && isMyTurn && (
      ...
      >
        ...
        <div className="overflow-y-auto h-[calc(100%-56px)] p-3 space-y-1.5">
          {log.length === 0 ? (
            ...
            log.map((entry, i) => (
              <div key={i} className="text-xs text-ink-muted py-1 border-b border-neutral-900">
  ...
  if (error && !resp) {
    return (
      <main className="min-h-screen bg-page flex items-center justify-center px-4">
        <div className="bg-surface border border-border-subtle rounded-xl p-8 max-w-md text-center">
          <p className="text-red-400 mb-4">{error}</p>
          <Link href="/play" className="text-accent-strong hover:text-accent-strong text-sm">
            &larr; Back to Lobby
          </Link>
        </div>
      </main>
  ...
  if (room?.status === "waiting" && isPlayer) {
    return (
      <main className="min-h-screen bg-page flex items-center justify-center px-4">
        <div className="bg-surface border border-border-subtle rounded-xl p-8 max-w-lg text-center space-y-5">
          <h2 className="text-2xl font-bold">Waiting for Opponent</h2>
          <p className="text-ink-muted">Share this room code:</p>
          <div className="bg-surface-elevated rounded-lg px-6 py-4">
            <span className="font-mono text-accent-strong text-4xl font-extrabold tracking-[0.3em]">
              {code}
            </span>
          </div>
          <button
          ...
          <Link href="/play" className="block text-ink-faint hover:text-ink-muted text-sm transition-colors">
            &larr; Back to Lobby
          </Link>
        </div>
      </main>
  ...
  if (room?.status === "waiting" || (room?.status === "playing" && !state?.player1?.leader && !state?.player2?.leader)) {
          ...
          <div className="bg-surface border border-border-subtle rounded-xl p-6 sm:p-8 max-w-xl w-full space-y-5">
            <div className="text-center">
              ...
              <p className="text-ink-muted text-sm">
                Room <span className="font-mono text-accent-strong font-bold">{code}</span>
    ...
    return (
      <main className="min-h-screen bg-page flex items-center justify-center px-4">
        <div className="bg-surface border border-border-subtle rounded-xl p-8 max-w-md text-center space-y-4">
          {isPlayer ? (
            <>
  ...
  if (room?.status === "finished" || state?.phase === "finished") {
    ...
    return (
      <main className="min-h-screen bg-page flex items-center justify-center px-4">
        <div className="bg-surface border border-border-subtle rounded-xl p-8 max-w-md text-center space-y-5">
          ...
          <div className="flex items-center justify-center gap-3 pt-2">
            <Link
              href="/play"
              className="bg-accent hover:bg-accent-strong text-black font-bold rounded-lg px-6 py-3 transition-colors"
            >
              Back to Lobby
            </Link>
          </div>
  ...
  return (
    <main className="min-h-screen bg-page text-ink flex flex-col">
      ...
      <header className="bg-surface/80 border-b border-border-subtle px-3 sm:px-4 py-2 flex items-center justify-between text-sm flex-shrink-0">
        <div className="flex items-center gap-3">
          <Link href="/play" className="text-ink-faint hover:text-ink-muted transition-colors">
            &larr;
          </Link>
          <span className="font-mono text-accent-strong font-bold">{code}</span>
            ...
            <div className="flex items-center gap-2">
              {state.lastUpkeepTurn !== state.turnNumber ? (
                <button
                  ...
                  className="text-xs bg-accent/20 text-accent-strong hover:bg-accent/30 px-3 py-1.5 rounded-lg transition-colors font-bold animate-pulse"
                >
      ...
      {isPlayer && isMyTurn && gameActive && (
        <div className="sm:hidden bg-surface border-t border-border-subtle px-3 py-2 flex items-center gap-2 overflow-x-auto flex-shrink-0">
          {state.lastUpkeepTurn !== state.turnNumber && (
            <button
              onClick={() => sendAction("begin_turn")}
              className="text-xs bg-accent/20 text-accent-strong px-3 py-2 rounded-lg whitespace-nowrap font-bold"
            >
              Start Turn
            </button>
          )}
      ...
      {/* Spectator banner */}
      {you === "spectator" && (
        <div className="bg-surface-elevated text-center text-ink-faint text-xs py-2 flex-shrink-0">
