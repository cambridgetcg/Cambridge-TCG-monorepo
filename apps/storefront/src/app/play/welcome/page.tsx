...
export default function PlayWelcome() {
  return (
    <div className="prose prose-invert max-w-3xl mx-auto py-12 px-4">
      <p className="not-prose mb-4 rounded-md border border-border-subtle bg-surface/40 p-3 text-xs text-ink-muted leading-relaxed">
        <span className="text-accent-strong">New to trading-card games?</span> Read{" "}
        <a href="/intro" className="text-accent-strong hover:text-accent-strong underline">/intro</a>{" "}
      ...
      <p className="border border-border-subtle bg-surface/40 rounded-md p-4 text-sm">
        ...
        <Link href="/methodology/play-module" className="text-accent-strong">
        ...
        <section key={archetype.id} className="my-12">
          ...
          <ul className="list-none p-0 space-y-4">
            {archetype.kinds.map((p) => (
              <li
                key={p.id}
                className="border border-border-subtle rounded-md p-4 bg-surface/40"
              >
                  ...
                  {p.recommended_steps.map((step) => (
                    <li key={step.href}>
                      <Link
                        ...
                        className="text-accent-strong hover:text-accent-strong font-medium"
                      >
                        {step.label}
                      </Link>
                      {step.note && (
