...
function PlaymatDiagram() {
  return (
    <div className="bg-page border border-border-subtle rounded-xl p-4 sm:p-6 my-6 font-mono text-[10px] sm:text-xs leading-tight overflow-x-auto">
...
function CardAnatomyDiagram() {
  return (
    <div className="bg-page border border-border-subtle rounded-xl p-4 sm:p-6 my-6 grid sm:grid-cols-2 gap-6">
      <div>
        <div className="text-ink-faint text-center text-[10px] uppercase tracking-wider mb-2">
          Leader card
        </div>
        <div className="bg-gradient-to-br from-red-900/40 to-red-950 border-2 border-danger/50 rounded-lg p-3 aspect-[5/7] flex flex-col font-mono text-[10px] text-ink">
          <div className="flex justify-between text-[9px]">
            <span className="bg-danger/30 px-1 rounded">Cost: —</span>
            <span className="bg-accent/30 px-1 rounded">5 life</span>
          </div>
          <div className="flex-1 flex items-center justify-center text-xs sm:text-sm text-accent-strong/80">
            ART
          </div>
          ...
          <div className="flex justify-between mt-1 text-[9px]">
            ...
            <span className="bg-danger/30 px-1 rounded">Power 5000</span>
      ...
      <div>
        ...
        <div className="bg-gradient-to-br from-blue-900/40 to-blue-950 border-2 border-blue-500/50 rounded-lg p-3 aspect-[5/7] flex flex-col font-mono text-[10px] text-ink">
          <div className="flex justify-between text-[9px]">
          ...
          <div className="text-[9px] text-ink-muted mb-1">Type / Trait</div>
          <div className="border-t border-blue-500/30 pt-1 text-[9px] text-ink-muted">
  ...
  return (
    <div className="grid sm:grid-cols-5 gap-2 my-6">
      {phases.map((p, i) => (
        ...
        >
          <div className="text-accent-strong text-xs font-bold mb-1">
            {i + 1}. {p.name}
          </div>
      ...
      {steps.map((s, i) => (
        ...
        >
          <div className="flex items-center gap-2 mb-2">
            <span className="bg-accent/20 text-accent-strong text-xs font-mono px-2 py-0.5 rounded">
              Step {i + 1}
            </span>
  ...
  return (
    <div className="space-y-4">
      ...
      {section.keywords_introduced.length > 0 && (
        <div className="text-[11px] text-ink-faint pt-2 border-t border-border-subtle/60">
          <span className="uppercase tracking-wider mr-2">Keywords</span>
          {section.keywords_introduced.map((kw) => (
            <span
              key={kw}
              className="inline-block bg-surface-elevated/60 text-ink-muted font-mono rounded px-1.5 py-0.5 mr-1.5 mb-1"
            >
              {kw}
            </span>
          ))}
      ...
      <section className="border-b border-neutral-900">
        <div className="mx-auto max-w-4xl px-4 py-10 sm:py-14">
          ...
          <h1 className="text-3xl sm:text-4xl font-extrabold tracking-tight mb-3">
            Never played a card game? <span className="text-accent-strong">Start here.</span>
          </h1>
          ...
          <div className="flex flex-wrap items-center gap-3 mt-6">
            <Link
              ...
              className="bg-accent hover:bg-accent-strong text-black font-bold rounded-lg px-5 py-2.5 text-sm transition-colors"
            >
            ...
            <Link
              ...
              className="text-xs text-ink-faint hover:text-ink-muted transition-colors font-mono ml-auto"
      ...
      <div className="mx-auto max-w-4xl px-4 py-10 grid lg:grid-cols-[200px_1fr] gap-10">
        ...
        <aside className="hidden lg:block">
          <div className="sticky top-20">
            ...
            <div className="text-[10px] text-neutral-600 mt-4 pt-4 border-t border-border-subtle/60 space-y-1">
              <Link
        ...
        <article className="space-y-12 min-w-0">
          {sections.map((s, i) => (
            <section
              key={s.id}
              className="bg-surface/40 border border-border-subtle rounded-xl p-5 sm:p-7"
            >
              <SectionHeader
                index={i + 1}
              ...
              <div className="flex items-center justify-between mt-6 pt-4 border-t border-border-subtle/60 text-sm">
                {i > 0 ? (
                  <Link
                ...
                {i < sections.length - 1 ? (
                  <Link
                    href={`#${sections[i + 1].id}`}
                    className="text-accent-strong hover:text-accent-strong transition-colors font-medium"
                  >
                    Next →
                  </Link>
                ) : (
                  <div className="flex flex-wrap items-center gap-3">
                    <Link
                      href="/play/starters"
                      className="bg-accent hover:bg-accent-strong text-black font-bold rounded-lg px-5 py-2 transition-colors"
                    >
                      Pick a starter deck →
                    </Link>
                    <Link
      ...
      <section className="border-t border-neutral-900">
        <div className="mx-auto max-w-4xl px-4 py-8 text-sm text-ink-faint space-y-2">
          <p>
            ...
            <Link href="/api/v1/play/tutorial" className="text-accent-strong/70 hover:text-accent-strong font-mono">
          ...
          <p>
            ...
            the{" "}
            <Link href="/api/v1/play/glossary" className="text-accent-strong/70 hover:text-accent-strong font-mono">
              bilingual glossary
            </Link>{" "}
          ...
          <p>
            ...
            <Link href="/play/compete" className="text-accent-strong/70 hover:text-accent-strong">
              /play/compete
            </Link>
            .
          </p>
          <p className="pt-2 italic">
            ...
            <Link href="/methodology/tutorial" className="text-accent-strong/70 hover:text-accent-strong">
