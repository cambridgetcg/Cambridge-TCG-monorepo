...
/** Buy routing info — tells the user where they're buying from. Every
 *  ask is a collector's; the reference price is context, not a rival offer. */
function BuyRoutingInfo({ view }: { view: UnifiedMarketView }) {
  ...
  if (reference_price != null && bestPrice < reference_price) {
    return (
      <div className="text-xs px-3 py-2 rounded-lg bg-bid/10 border border-bid/20 text-bid">
        Buy from seller at <Money value={bestPrice} className="font-mono tabular-nums" /> (<Money value={reference_price - bestPrice} className="font-mono tabular-nums" /> below the reference price)
      </div>
    );
  }
