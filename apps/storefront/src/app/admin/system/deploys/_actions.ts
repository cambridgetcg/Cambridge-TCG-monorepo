...
import { PROJECTS, deployFromGit, type VercelProject } from "@/lib/admin/vercel";
...
export async function redeployFromMain(input: { projectKey: VercelProject["key"] }) {
  return adminAction({
    ...
    revalidate: "/admin/system/deploys",
    run: async () => {
      ...
      // working-tree state because the storefront server doesn't have one.
      const ghToken = process.env.GITHUB_TOKEN;
      if (!ghToken) {
        throw new ActionInputError(
          "GITHUB_TOKEN env var is required to resolve main HEAD. " +
          "Set in apps/storefront/.env.local and the Vercel project env.",
        );
      }
