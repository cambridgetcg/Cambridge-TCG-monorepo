 ...
 *
 * Closes the governance triangle: /admin/system/audit (read), /admin/system/admin
...
export default async function Page({
  ...
  // not deployed). Matches /admin/system/audit&apos;s data source.
  ...
  const buildHref = (overrides: Record<string, string | undefined>): string => {
    ...
    return `/admin/system/admin${qs ? `?${qs}` : ""}`;
  };
  ...
  return (
    <div className="max-w-5xl space-y-6">
      <PageHeader
        title="Admin Users"
        description="Storefront users with role=&apos;admin&apos;. Grants and revocations write to admin_actions_log — every change is auditable from /admin/system/audit. You cannot revoke your own role (lockout protection)."
        action={
          <Link
  ...
  return (
    <div className="max-w-5xl space-y-6">
      ...
      <FilterPills
        selected="candidates"
        pills={[
          { value: "current",    label: "Current admins", count: "—", href: "/admin/system/admin" },
          { value: "candidates", label: "Grant new",      count: "—", href: "/admin/system/admin?tab=candidates" },
        ]}
      />
      ...
      <SearchForm
        ...
        clearHref="/admin/system/admin?tab=candidates"
