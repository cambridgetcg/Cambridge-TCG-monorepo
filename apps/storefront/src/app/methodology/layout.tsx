  ...
  return (
    <div className="min-h-screen bg-page">
      <div className="max-w-3xl mx-auto px-4 py-8">
        <Link
          href="/methodology"
          className="inline-block text-xs text-ink-faint hover:text-accent-strong mb-6 transition uppercase tracking-wider"
        >
          ← All methodology pages
        </Link>
        <article className="prose prose-invert max-w-none prose-headings:text-ink prose-p:text-ink-muted prose-li:text-ink-muted prose-strong:text-ink prose-a:text-accent-strong hover:prose-a:text-accent-strong prose-code:text-accent-strong prose-code:bg-surface prose-code:px-1.5 prose-code:py-0.5 prose-code:rounded prose-code:text-[0.85em] prose-code:font-mono">
          {children}
        </article>
      </div>
    </div>
