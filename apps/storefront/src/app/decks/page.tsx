          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {filtered.map((d) => (
              <Link
                key={d.id}
                href={`/decks/${d.slug}`}
                className="group bg-surface border border-border-subtle rounded-lg overflow-hidden hover:border-accent transition-colors"
              >
                  ...
                  {d.tags.length > 0 && (
                    <div className="flex flex-wrap gap-1 mt-2">
                      {d.tags.slice(0, 3).map((t) => (
                        <span
                          key={t}
                          className="text-[9px] bg-surface-subtle text-ink-muted px-1.5 py-0.5 rounded uppercase tracking-wide"
                        >
                          {t}
                        </span>
                      ))}
