        ...
        <div className="mx-auto max-w-5xl px-4 py-12">
          ...
          <div className="mt-6 flex items-center gap-3 flex-wrap">
            <input
              ...
              className="bg-surface border border-border-subtle rounded-lg px-4 py-2.5 text-sm w-full sm:w-80 focus:outline-none focus:border-accent"
            />
            <Link
              href="/deck-builder"
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {filtered.map((d) => (
              <Link
                key={d.id}
                href={`/decks/${d.slug}`}
                className="group bg-surface border border-border-subtle rounded-xl overflow-hidden hover:border-accent/40 transition-colors"
              >
                <div className="aspect-[5/7] relative bg-page">
                ...
                <div className="p-3">
                  <p className="font-semibold text-sm truncate group-hover:text-accent-strong transition-colors">
                    {d.name}
                  </p>
                  {d.leader_card && (
                  ...
                  {d.tags.length > 0 && (
                    <div className="flex flex-wrap gap-1 mt-2">
                      {d.tags.slice(0, 3).map((t) => (
                        <span
                          key={t}
                          className="text-[9px] bg-surface-elevated text-ink-muted px-1.5 py-0.5 rounded uppercase tracking-wide"
                        >
                          {t}
                        </span>
                      ))}
