...
function rarityBadge(rarity: string | null) {
  ...
  if (r === "SR" || r === "SEC" || r === "L" || r === "SP") cls = "bg-warning/15 text-warning";
  else if (r === "R") cls = "bg-[#6a5a8f]/15 text-[#6a5a8f]";
  else if (r === "UC") cls = "bg-info/10 text-info";
  return (
  ...
  return (
    <main className="min-h-screen bg-page text-ink">
      <div className="mx-auto max-w-5xl px-4 py-8">
        <Link href="/decks" className="text-sm text-ink-faint hover:text-ink">
        ...
        {notFound && (
          <div className="mt-8 bg-danger/10 border border-danger/40 text-danger rounded-lg px-4 py-3 text-sm">
        ...
        {deck && (
          <>
            <div className="mt-4 mb-6 flex items-start justify-between gap-4 flex-wrap">
              <div>
                <h1 className="text-2xl sm:text-3xl font-display font-semibold tracking-tight">{deck.name}</h1>
                ...
                {deck.tags.length > 0 && (
                  <div className="flex flex-wrap gap-1 mt-2">
                    {deck.tags.map((t) => (
                      <span
                        key={t}
                        className="text-[10px] bg-surface-subtle text-ink-muted px-2 py-0.5 rounded uppercase tracking-wide"
                      >
                        {t}
                      </span>
                    ))}
              ...
              <div className="flex items-center gap-2">
                <button
                  onClick={copyAsText}
                  className="bg-surface hover:bg-surface border border-border-subtle text-sm font-medium rounded-lg px-4 py-2 transition-colors"
                >
                  {copied ? "Copied!" : "Copy deck list"}
                </button>
                <Link
                  href="/deck-builder"
                  className="bg-ink hover:bg-ink/85 text-page font-bold text-sm rounded-lg px-4 py-2 transition-colors"
                >
                  Build your own
                </Link>
              </div>
            ...
            {deck.notes && (
              <div className="mb-6 bg-surface-subtle border border-border-subtle rounded-lg p-4">
              ...
              <div className="space-y-4">
                {leader && (
                  <div className="bg-surface border border-border-subtle rounded-lg overflow-hidden">
                    ...
                    <div className="p-3">
                      <p className="text-[10px] uppercase tracking-wider text-accent font-bold">Leader</p>
              ...
              <div className="bg-surface border border-border-subtle rounded-lg overflow-hidden">
                ...
                <div className="divide-y divide-border-subtle">
                  {mainDeck.map((e) => (
                    ...
                    >
                      <div className="relative w-10 h-14 flex-shrink-0 rounded overflow-hidden bg-surface-subtle">
                        {e.card.image_url && (
                      ...
                      <span className="text-accent font-bold text-sm w-10 text-right">
                        ×{e.quantity}
                      </span>
                    </div>
                  ))}
