        ...
        return (
          <section key={kind}>
            ...
            {list.map((node) => {
              ...
              return (
                <article key={node.id} className="mb-6 pl-3 border-l border-border-subtle">
                  <h3 className="text-base">
                    <code className="text-accent-strong">{node.id}</code>{" "}
                    <span className="text-ink">— {node.label}</span>
                  </h3>
                  {node.description && (
                    <p className="text-sm text-ink-muted my-1">{node.description}</p>
                  )}
                  {node.path && (
                    <p className="text-xs text-ink-faint my-1">
                      ...
                      <ul className="text-sm">
                        {out.map((e, i) => (
                          <li key={i}>
                            ...
                            <code className="text-ink-muted">{e.to}</code>{" "}
                  ...
                  {inc.length > 0 && (
                    <div className="mt-2">
                      ...
                      <ul className="text-sm">
                        {inc.map((e, i) => (
                          <li key={i}>
                            <code className="text-ink-muted">{e.from}</code>{" "}
                            <span className="text-xs text-ink-faint">
                              ({labelFor(e.from)})
                            </span>{" "}
