...
function CoverageStatusPanel({ slug }: { slug: string }) {
  ...
  return (
    ...
    <details className="rounded-lg border border-border-subtle bg-page">
        ...
        <div>
          ...
          <ul className="space-y-1 text-sm">
            ...
            {anticipated.length > 0 && (
              <li className="text-accent-strong">
            ...
            <ul className="space-y-1 text-sm">
              {openGaps.map((g) => (
                <li key={g.id} className="text-ink-muted">
                  <span className="mr-1 inline-block rounded bg-amber-950 px-1.5 py-0.5 text-[10px] text-accent-strong ring-1 ring-amber-800">
                    {g.status}
                  </span>
                  {g.name}
                </li>
      ...
      {!ctx.confirmed && (
        <p className="mt-4 rounded border border-amber-900 bg-amber-950/40 p-3 text-xs text-accent-strong">
      ...
      <main className="max-w-6xl mx-auto px-4 py-12">
        ...
        <div className="mb-4 flex flex-wrap items-center gap-3">
          ...
          {cfg.cardrush && !cfg.cardrush.confirmed && (
            <span
              className="inline-block text-[10px] uppercase tracking-wider px-2 py-0.5 bg-accent/10 text-accent-strong border border-accent/30 rounded"
        ...
        <form
          ...
          method="get"
          className="mb-10 rounded-lg border border-amber-700/50 bg-accent/5 p-4 grid grid-cols-1 sm:grid-cols-[1fr_auto] gap-3 items-end"
        >
          ...
          <div>
            <label className="block text-xs uppercase tracking-wider text-accent-strong mb-2 font-semibold">
          ...
          <button
            type="submit"
            className="rounded-md bg-accent px-5 py-2 text-sm font-semibold text-black hover:bg-accent-strong transition"
          >
            Search →
          </button>
        </form>
        ...
        {gameCoverage && gameCoverage.observations > 0 && (
          <section className="mb-10 rounded-lg border border-border-subtle bg-surface p-4">
            ...
            {gameCoverageSources.length > 1 && (
              <div className="text-[10px] text-ink-faint pt-3 border-t border-border-subtle/60">
            ...
            return (
              <>
                <div className="flex items-baseline justify-between gap-4 mb-5 flex-wrap">
                  ...
                  {empty.length > 0 && (
                    <span
                      className="text-[10px] uppercase tracking-wider px-2 py-0.5 bg-surface-elevated/60 text-ink-faint border border-border-subtle rounded"
        ...
        <section className="border-t border-border-subtle pt-8">
          ...
          <p className="text-ink-muted text-sm leading-relaxed max-w-3xl mb-4">
            {cfg.pricing_note}{" "}
            The <strong className="text-ink">Buy Price</strong> is our
