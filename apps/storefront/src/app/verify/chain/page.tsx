  ...
  return (
    <main className="min-h-screen bg-page text-ink">
      ...
      <div className="max-w-4xl mx-auto px-4 py-12">
        ...
        {/* ── The tip ──────────────────────────────────────────────── */}
        <section className="bg-surface border border-accent/30 rounded-xl p-6 mb-6">
          ...
          {tip ? (
            <>
              ...
              <code className="block font-mono text-sm text-accent-strong bg-page rounded p-3 break-all">
        ...
        <section className="mb-6">
          ...
          <p className="text-sm text-ink-faint mb-4">
            Most recent first. Each row&apos;s <code className="font-mono text-accent-strong">chain_hash</code> = SHA-256(<code className="font-mono">prev_hash</code> || <code className="font-mono">root</code>).
                ...
                <tbody>
                  {recent.map((d) => (
                    <tr key={d.id} className="border-b border-border-subtle last:border-0">
                      <td className="px-4 py-3 font-mono text-accent-strong">{d.id}</td>
                      ...
                      <td className="px-4 py-3 font-mono text-xs text-secondary" title={d.chain_hash ?? ""}>
                  ...
                  {recent.length > 0 && recent[recent.length - 1].id <= 50 && (
                    <tr className="border-t border-border-subtle/60 bg-page/40">
          ...
          <p className="text-xs text-ink-faint mt-3">
            Older digests via <Link href="/api/verify/chain?from_id=0" className="text-accent-strong hover:text-accent-strong underline">/api/verify/chain</Link> (paginated, JSON, CORS-enabled).
          </p>
        </section>

        {/* ── How to verify ───────────────────────────────────────── */}
        <section className="bg-surface border border-border-subtle rounded-xl p-6 mb-6">
          ...
          <ol className="list-decimal list-inside space-y-2 text-sm text-ink-muted">
            ...
            <li>Walk forward from any prior digest by recomputing
              <code className="font-mono text-accent-strong mx-1">chain_hash = SHA-256(prev_hash || root)</code>.</li>
        ...
        <section className="grid sm:grid-cols-2 gap-3 text-sm">
          <Link
            ...
            className="bg-surface border border-border-subtle hover:border-border-strong rounded-xl p-4 transition-colors"
          >
