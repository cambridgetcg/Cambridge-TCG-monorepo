...
export default function VerifyDrawPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (error) return <Shell><div className="bg-danger/10 border border-danger/30 rounded-xl p-6 text-center"><p className="text-red-400 font-bold">{error}</p></div></Shell>;
  ...
  return (
    <Shell>
      ...
      <header className="mb-8">
        <Link href="/verify" className="text-xs text-ink-faint hover:text-ink-muted">← All proofs</Link>
      ...
      <div className="grid lg:grid-cols-2 gap-6 mt-6">
        ...
        <section className="bg-surface border border-border-subtle rounded-xl p-4 lg:col-span-2">
            ...
            <tbody>
              {verdict?.slots.map((s) => (
                <tr key={s.slotIndex} className="border-t border-border-subtle">
                  ...
                  <td className={`py-1.5 font-mono uppercase ${s.matches ? "text-ink-muted" : "text-red-400"}`}>{s.recomputedPicked}</td>
                  <td className="py-1.5 font-mono text-ink-faint">{s.recomputedRoll.toFixed(10)}</td>
                  <td className="py-1.5 text-right">
                    {s.matches ? <span className="text-secondary">✓</span> : <span className="text-red-400">✗</span>}
                  </td>
                </tr>
              ))}
            </tbody>
...
function VerdictBanner({ verdict }: { verdict: Verdict | null }) {
  if (!verdict) {
    return <div className="rounded-xl p-4 bg-surface border border-border-subtle"><p className="text-sm text-ink-faint">Computing…</p></div>;
  }
  return (
    <div className={`rounded-xl p-4 border flex items-center gap-3 ${
      ...
      <span className={`text-3xl ${verdict.allMatch ? "text-secondary" : "text-red-400"}`}>
        {verdict.allMatch ? "✓" : "✗"}
      </span>
      <div>
...
function CheckCard({ ok, title, explanation, children }: {
  ...
  return (
    <section className={`bg-surface border rounded-xl p-4 ${
      ok === true ? "border-emerald-500/30" : ok === false ? "border-danger/40" : "border-border-subtle"
    }`}>
      <div className="flex items-baseline gap-2 mb-2">
        <span className={`text-lg ${ok === true ? "text-secondary" : ok === false ? "text-red-400" : "text-ink-faint"}`}>
          {ok === true ? "✓" : ok === false ? "✗" : "…"}
        </span>
...
function Field({ label, value, mono }: { label: string; value: string; mono?: boolean }) {
  return (
    <div className="text-xs">
      ...
      <span className={`text-ink break-all ${mono ? "font-mono" : ""}`}>{value}</span>
    </div>
  );
}
