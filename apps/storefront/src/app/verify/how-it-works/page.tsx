  ...
  -d '{
    ...
    "claimed_rarity": "common"
        ...
        <Section title="Why you should still be sceptical">
          ...
          <p>
            The <Link href="/verify/fairness" className="text-accent-strong hover:text-accent-strong underline">aggregate fairness dashboard</Link>{" "}
...
function DataRow({ field, status, reason }: { field: string; status: string; reason: string }) {
  ...
  return (
    <tr className="border-t border-border-subtle first:border-t-0">
      ...
      <td className={`py-2 pr-3 text-xs whitespace-nowrap ${isPrivate ? "text-red-400" : "text-secondary"}`}>
        {status}
      </td>
