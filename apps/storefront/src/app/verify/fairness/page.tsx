  ...
  return (
    <main className="min-h-screen bg-page text-ink">
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="mt-10 text-xs text-neutral-600 border-t border-border-subtle pt-6">
          <p className="mb-1">
...
function TierCard({ tier }: { tier: TierReport }) {
  ...
  return (
    <section className={`bg-surface border rounded-xl p-5 ${
      tier.enough_samples ? "border-border-subtle" : "border-neutral-900 opacity-80"
    }`}>
...
function RarityRow({ rarity, signalEnabled }: { rarity: TierRarity; signalEnabled: boolean }) {
  ...
  return (
    <tr className="border-t border-border-subtle">
      ...
      <td className={`py-1.5 text-right font-mono ${warn ? "text-accent-strong" : "text-ink-muted"}`}>
...
function DrawKindCard({ report }: { report: DrawKindReport }) {
  return (
    <section className={`bg-surface border rounded-xl p-5 ${
      report.enough_samples ? "border-border-subtle" : "border-neutral-900 opacity-80"
    }`}>
      <div className="flex items-baseline justify-between mb-3 flex-wrap gap-2">
...
function DistributionBar({ expected, observed }: { expected: number; observed: number }) {
  ...
  return (
    <div className="relative h-4 bg-page rounded overflow-hidden">
      ...
      {/* Observed (amber line) */}
      <div
        className="absolute inset-y-0 left-0 border-r-2 border-accent-strong"
        style={{ width: `${obsPct}%` }}
      />
    </div>
  );
