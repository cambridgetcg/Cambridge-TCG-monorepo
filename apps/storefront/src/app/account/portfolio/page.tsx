    ...
    <div>
      ...
      {summary && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
          <div className="bg-surface rounded-lg p-4">
            ...
            <p className="text-2xl font-bold text-accent"><Money value={summary.total_value} /></p>
          </div>
          <div className="bg-surface rounded-lg p-4">
          ...
          <div className="bg-surface rounded-lg p-4">
            ...
            {summary.total_pnl != null ? (
              <p className={`text-lg font-semibold ${summary.total_pnl >= 0 ? "text-bid" : "text-ask"}`}>
                {summary.total_pnl >= 0 ? "+" : ""}
          ...
          <div className="flex gap-2">
            {profile.username && profile.is_public && (
              <button
                ...
                className="text-xs bg-ink hover:opacity-90 text-page font-semibold rounded px-3 py-1.5 transition-colors whitespace-nowrap"
              >
      ...
      {cards.length === 0 ? (
        <div className="bg-surface rounded-lg p-12 text-center">
          ...
          <Link
            href="/account/portfolio/add"
            className="inline-block px-5 py-2.5 bg-ink text-page font-semibold rounded-lg hover:opacity-90 transition text-sm"
          >
            Add Cards
          </Link>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {sorted.map((card) => (
            <div key={card.id} className="bg-surface rounded-lg overflow-hidden flex flex-col">
              {/* Image */}
              ...
              <div className="p-3 flex-1 flex flex-col">
                ...
                <div className="flex gap-1.5 mb-2">
                  {card.rarity && (
                    <span className="text-[10px] px-1.5 py-0.5 rounded bg-accent-wash text-accent font-medium uppercase">
                      {card.rarity}
                    </span>
                  )}
                  <span className="text-[10px] px-1.5 py-0.5 rounded bg-surface-subtle text-ink-muted font-medium">
                    {card.condition}
                  </span>
                </div>
                    ...
                    return (
                      <span
                        ...
                        className={`text-[10px] font-mono px-1.5 py-0.5 rounded ${
                          positive ? "bg-bid/15 text-bid" : "bg-ask/15 text-ask"
                        }`}
                ...
                <div className="text-xs mb-3">
                  {card.total_cost != null ? (
                    <>
                      ...
                      {card.pnl != null && (
                        <span className={`ml-1.5 font-medium ${card.pnl >= 0 ? "text-bid" : "text-ask"}`}>
                    ...
                    <span className="text-ink-faint">No cost basis</span>
                  )}
                </div>
                ...
                <div className="mt-auto flex gap-1.5">
                  <button
                    ...
                    className="flex-1 text-xs py-1.5 rounded-lg border border-border-subtle text-ink-muted hover:text-ink hover:border-border-strong transition"
                  >
                    {editingId === card.id ? "Cancel" : "Edit"}
                  </button>
                  <button
                    onClick={() => removeCard(card.id)}
                    className="text-xs px-3 py-1.5 rounded-lg border border-border-subtle text-ink-faint hover:text-danger hover:border-danger/30 transition"
                  >
                    Remove
                  </button>
                </div>
                ...
                {editingId === card.id && (
                  <div className="mt-3 pt-3 border-t border-border-subtle space-y-2">
                    <div className="flex gap-2">
                      <div className="flex-1">
                        ...
                        <input
                          ...
                          className="w-full bg-surface-subtle border border-border-subtle rounded px-2 py-1 text-sm text-ink"
                        />
                      </div>
                      <div className="flex-1">
                    ...
                    <button
                      ...
                      disabled={saving}
                      className="w-full py-1.5 bg-ink text-page font-semibold rounded-lg text-sm hover:opacity-90 transition disabled:opacity-50"
                    >
