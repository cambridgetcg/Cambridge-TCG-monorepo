    ...
    <div>
      ...
      <div className="relative mb-6">
        <input
          ...
          className="w-full bg-surface border border-border-subtle rounded-xl px-4 py-3 text-ink placeholder:text-neutral-600 focus:outline-none focus:border-accent/50 transition"
          autoFocus
        />
        {searching && (
      ...
      {results.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {results.map((card) => (
            <div
              key={card.sku}
              className={`bg-surface rounded-xl overflow-hidden transition cursor-pointer border ${
            ...
            >
              ...
              <div className="flex gap-3 p-3">
                <div className="relative w-16 h-[90px] bg-surface-elevated rounded shrink-0 overflow-hidden">
                  {card.image_url ? (
                    <Image
                      src={card.image_url}
                      alt={card.card_name}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {card.rarity && (
                    <span className="inline-block mt-1 text-[10px] px-1.5 py-0.5 rounded bg-accent/15 text-accent-strong font-medium uppercase">
                      {card.rarity}
                    </span>
                  )}
                  {card.price != null && (
                    <p className="text-sm font-semibold text-accent-strong mt-1">{formatPrice(card.price)}</p>
                  )}
                </div>
              </div>
              ...
              {selectedSku === card.sku && (
                ...
                >
                  <div className="flex gap-2">
                    <div className="flex-1">
                      ...
                      <input
                        ...
                        className="w-full bg-surface-elevated border border-border-strong rounded px-2 py-1.5 text-sm text-ink"
                      />
                    </div>
                    <div className="flex-1">
                  ...
                  <button
                    ...
                    disabled={adding}
                    className="w-full py-2 bg-accent text-black font-semibold rounded-lg text-sm hover:bg-accent-strong transition disabled:opacity-50"
                  >
