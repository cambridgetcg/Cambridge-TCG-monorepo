...
export default function PortfolioValuePage() {
  ...
  if (error || !value) {
    return (
      <div className="bg-danger/10 border border-danger/30 rounded-lg p-3 text-sm text-red-300">
  ...
  return (
    <div>
      <div className="flex items-baseline justify-between gap-4 flex-wrap mb-2">
        <h1 className="text-2xl font-black text-ink">Collection Value</h1>
        <button
          onClick={exportCertificate}
          disabled={exporting || isEmpty}
          className="text-xs px-3 py-1.5 bg-surface-elevated text-ink hover:bg-neutral-700 rounded-lg font-medium disabled:opacity-50 transition"
        >
        ...
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
            <div className="bg-surface rounded-xl p-4 border border-accent/20">
              <div className="text-[10px] uppercase tracking-wide text-accent-strong/80">Total value</div>
              <div className="text-2xl font-black text-ink mt-1">{fmt(value.total_value)}</div>
            ...
            <div className="bg-surface rounded-xl p-4 border border-border-subtle">
              ...
              <div className="text-2xl font-black text-ink-muted mt-1">{fmt(value.total_cost)}</div>
              <div className="text-[10px] text-ink-faint mt-1">
            ...
            <div className="bg-surface rounded-xl p-4 border border-border-subtle">
              ...
              <div className="text-2xl font-black text-ink mt-1">{value.card_count}</div>
              <div className="text-[10px] text-ink-faint mt-1">
          ...
          {/* Time-series chart */}
          {series.length > 1 && (
            <div className="bg-surface rounded-xl p-5 mb-6 border border-border-subtle">
                  ...
                  return (
                    <div key={s.set_code} className="flex items-center gap-3 text-xs">
                      <Link
                        ...
                        className="w-32 truncate text-ink-muted hover:text-accent-strong transition"
                      >
              ...
              <div className="space-y-1">
                {value.top_cards.map((c) => (
                  <Link
                    ...
                    className="flex items-center gap-3 text-xs py-1.5 hover:bg-surface-elevated/50 rounded px-2 transition"
                  >
          ...
          {value.unpriced_sku_count > 0 && (
            <div className="bg-accent/10 border border-accent/20 rounded-lg p-3 text-xs text-accent-strong">
  ...
  return (
    <div>
      ...
      <div className="flex justify-between text-[10px] text-ink-faint mt-1">
        ...
        <span className={delta >= 0 ? "text-secondary" : "text-red-400"}>
