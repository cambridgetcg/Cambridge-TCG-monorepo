    ...
    <div>
      ...
      <div className="flex items-baseline justify-between flex-wrap gap-3 mb-6">
        ...
        {unreadCount > 0 && (
          <button
            onClick={markAllRead}
            disabled={marking}
            className="text-sm text-accent-strong hover:text-accent-strong transition disabled:opacity-50"
          >
      ...
      {loading && items.length === 0 ? (
        ...
        <div className="bg-surface rounded-xl p-8 text-center">
          ...
          <Link href="/" className="text-sm text-accent-strong hover:text-accent-strong">Back to site</Link>
        </div>
      ) : (
        <>
          <ul className="space-y-2">
            {items.map((n) => {
              ...
              const card = (
                <div
                  className={`bg-surface hover:bg-surface/60 border rounded-xl p-4 transition ${
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-3 mt-2 text-[11px] text-neutral-600">
                        ...
                        <code className="px-1.5 py-0.5 rounded bg-surface-elevated text-ink-faint">{n.kind}</code>
                      </div>
                    </div>
                    {unread && (
                      <button
          ...
          {hasMore && (
            <div className="mt-6 flex justify-center">
              <button
                onClick={() => load(false)}
                disabled={loading}
                className="px-5 py-2.5 bg-surface border border-border-subtle text-sm text-ink-muted rounded-lg hover:bg-surface-elevated transition disabled:opacity-50"
              >
