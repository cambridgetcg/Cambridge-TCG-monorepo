...
const STATUS_COPY: Record<string, { label: string; tone: string; advice: string }> = {
  needs_response: {
    ...
    tone: "bg-accent-wash text-accent border-accent/30",
  ...
  warning_closed: {
    ...
    tone: "bg-surface-subtle text-ink-muted border-border-subtle",
  ...
  return (
    <div>
      ...
      <p className="text-sm text-ink-muted mb-6">
        ...
        <a href="mailto:support@cambridgetcg.com" className="text-accent underline">support@cambridgetcg.com</a>{" "}
      ...
      {loading ? (
        ...
        <div className="bg-ok/5 border border-ok/30 rounded-lg p-6 text-center">
          <p className="text-ok font-bold mb-1">No chargebacks on file ✓</p>
          <p className="text-xs text-ink-faint">
            See your{" "}
            <Link href="/account/standing" className="text-accent underline">account standing</Link>{" "}
        ...
        <div className="space-y-3">
          {chargebacks.map((c) => {
            ...
            return (
              <div key={c.stripe_dispute_id} className={`rounded-lg border p-4 ${copy.tone}`}>
