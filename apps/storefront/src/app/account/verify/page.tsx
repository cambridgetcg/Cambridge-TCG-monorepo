  ...
  if (loggedIn === false) {
    return (
      <div className="bg-surface rounded-xl p-8 text-center">
        ...
        <a href="/login" className="text-accent-strong hover:underline text-sm font-medium">Sign in</a>
      </div>
    );
  }
        ...
        <div className="bg-surface rounded-xl p-6 space-y-4">
                ...
                <div key={doc.id} className="relative group">
                  <a href={doc.url} target="_blank" rel="noopener noreferrer"
                    ...
                    {doc.mime_type?.startsWith("image/") ? (
                      ...
                      <div className="aspect-square w-full bg-surface-elevated flex items-center justify-center text-ink-faint text-xs">PDF</div>
                    )}
                  ...
                  <button
                    ...
                    className="absolute top-1 right-1 w-6 h-6 rounded-full bg-black/70 text-ink-muted hover:bg-red-900/80 hover:text-ink opacity-0 group-hover:opacity-100 transition text-xs"
                    title="Remove"
                  >×</button>
                </div>
              ))}
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <select
              ...
              className="px-3 py-2 bg-surface-elevated border border-border-strong rounded-lg text-sm text-ink focus:outline-none focus:border-accent/50"
            >
        ...
        <>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <button
              type="submit"
              disabled={submitting}
              className="w-full py-3 rounded-lg font-bold text-sm bg-accent text-black hover:bg-accent-strong transition disabled:opacity-50"
            >
              {submitting
                ? "Submitting…"
                : status === "rejected"
...
function StatusCard({
  ...
  const cls = {
    ...
    amber:   "bg-accent/10 border-accent/30 text-accent-strong",
    red:     "bg-danger/10 border-danger/30 text-red-300",
  }[tone];
  return (
  ...
  return (
    <div>
      ...
      <input
        ...
        className={`w-full px-3 py-2.5 bg-surface-elevated border rounded-lg text-ink text-sm focus:outline-none transition ${
          error
            ? "border-danger/50 focus:border-danger"
            : "border-border-strong focus:border-accent/50"
        } ${className ?? ""}`}
      />
