...
const SEVERITY_TONE: Record<string, string> = {
  critical: "bg-danger/10 text-red-400 border-danger/30",
  high:     "bg-accent/10 text-accent-strong border-accent/30",
  medium:   "bg-sky-500/10 text-info border-sky-500/30",
  low:      "bg-surface-elevated text-ink-muted border-border-strong",
};
    ...
    <div>
      ...
      <div className={`rounded-xl p-5 mb-6 border ${
        standing.is_suspended ? "border-danger/40 bg-danger/5"
          : flags.length > 0 ? "border-accent/40 bg-accent/5"
          : "border-emerald-500/40 bg-emerald-500/5"
      }`}>
        <div className="flex items-center gap-3">
          <span className={`text-3xl ${
            standing.is_suspended ? "text-red-400"
              : flags.length > 0 ? "text-accent-strong"
              : "text-secondary"
          }`}>
        ...
        <section>
          ...
          <div className="space-y-3">
            {flags.map((f) => {
              ...
              return (
                <div key={f.id} className={`rounded-xl border p-4 ${SEVERITY_TONE[f.severity] ?? "border-border-subtle bg-surface"}`}>
          ...
          <div className="mt-6 p-4 bg-surface border border-border-subtle rounded-xl text-sm text-ink-muted">
            <p>
              ...
              <a href="mailto:support@cambridgetcg.com" className="text-accent-strong underline">
      ...
      {goodStanding && (
        <div className="text-center text-sm text-ink-faint mt-8">
          Your trust score is{" "}
          <Link href="/account/trust" className="text-secondary underline">
