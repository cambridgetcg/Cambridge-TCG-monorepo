...
function VaultItemRow({
  ...
  return (
    <div className="bg-surface rounded-lg overflow-hidden">
      <button
        onClick={onToggle}
        className="w-full flex items-center gap-3 px-3 py-3 text-left hover:bg-surface-subtle transition"
      >
        <div className="relative w-10 h-14 shrink-0 rounded overflow-hidden bg-surface-subtle">
          {item.image_url && (
          ...
          {item.status === "redeemed" && item.tracking_number && (
            <div>
              ...
              <p className="text-sm">
                ...
                {trackUrl ? (
                  <a href={trackUrl} target="_blank" rel="noopener noreferrer" className="text-accent underline font-mono">
          ...
          {item.bounty_pull_id && (
            <div>
              ...
              <Link
                ...
                target="_blank"
                className="inline-flex items-center gap-1 text-xs bg-ok/10 hover:bg-ok/20 border border-ok/30 text-ok rounded px-2 py-1 transition-colors"
              >
                ✓ Verify this pull ↗
              </Link>
            </div>
