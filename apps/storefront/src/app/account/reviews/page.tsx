  ...
  return (
    <div>
      ...
      {data.received.length > 0 && (
        <div className="bg-surface border border-border-subtle rounded-xl p-4 mb-6 flex items-center gap-4 flex-wrap">
          <div>
      ...
      <div className="flex gap-2 mb-4">
        <button
          ...
          className={`text-xs px-3 py-1.5 rounded-lg transition-colors ${
            tab === "received" ? "bg-accent text-black font-bold"
        ...
        <button
          ...
          className={`text-xs px-3 py-1.5 rounded-lg transition-colors ${
            tab === "given" ? "bg-accent text-black font-bold"
              : "bg-surface text-ink-muted hover:text-ink hover:bg-surface-elevated"
          }`}
        >
      ...
      {list.length === 0 ? (
        ...
        <div className="space-y-3">
          {list.map((r) => {
            ...
            return (
              <div key={r.id} className={`bg-surface rounded-xl p-4 border ${
                r.admin_hidden ? "border-border-subtle/60 opacity-70" : "border-border-subtle"
              }`}>
                ...
                <div className="text-xs text-ink-faint">
                  ...
                  {counterpartyUsername ? (
                    <Link href={`/u/${counterpartyUsername}`} className="text-accent-strong hover:text-accent-strong underline">
                      {counterparty}
                    </Link>
                  ) : counterparty}
                </div>
                ...
                {isReceived && r.admin_hidden && (
                  <div className="mt-3 bg-accent/5 border border-accent/30 rounded p-3 text-xs">
                    <p className="text-accent-strong font-bold mb-1">This review is hidden from public view</p>
                    {appealedAlready ? (
                      ...
                      <button
                        ...
                        className="mt-1 text-xs bg-accent/20 hover:bg-accent/30 text-accent-strong rounded px-3 py-1.5 disabled:opacity-50"
                      >
                        Appeal this hide
                      </button>
                    ) : null}
