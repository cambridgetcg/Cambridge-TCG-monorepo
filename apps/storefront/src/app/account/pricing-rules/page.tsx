...
export default function PricingRulesPage() {
  ...
  return (
    <div>
      ...
      <h1 className="text-2xl font-display font-semibold text-ink mb-2">Pricing Rules</h1>
      ...
      {loading ? (
        ...
        <div className="bg-surface rounded-lg p-8 text-center">
          ...
          <Link href="/account/offers" className="inline-block mt-3 text-accent text-xs font-semibold hover:text-accent-strong">
  ...
  return (
    <div className="bg-surface rounded-lg p-4 border border-border-subtle">
      ...
      <div className="bg-surface-subtle rounded p-2 mb-2">
        <p className={`text-xs ${rule.rule_type === "auto_counter" ? "text-info" : "text-accent"}`}>
          {action}
        </p>
      ...
      <div className="flex items-center gap-2 mt-3 flex-wrap">
        ...
        {rule.status !== "archived" && (
          <button
            ...
            className="px-3 py-1.5 text-xs font-medium text-danger hover:text-danger transition"
          >
            Archive
          </button>
        )}
...
function NewRuleForm({ onCreated }: { onCreated: () => void }) {
  ...
  return (
    <div className="bg-surface rounded-lg border border-accent/30 p-5 mb-4">
      <h2 className="text-sm font-bold text-accent uppercase tracking-wide mb-3">New rule</h2>
          ...
          <button
            ...
            className={`text-left px-3 py-2 rounded-lg border transition ${
              ruleType === v
                ...
                : "border-border-subtle bg-surface-subtle hover:bg-surface-subtle"
            }`}
          >
            <div className={`text-sm font-bold ${ruleType === v ? "text-accent" : "text-ink-muted"}`}>
              {label}
            </div>
      ...
      <input
        ...
        maxLength={200}
        className="w-full px-3 py-2 mb-4 bg-surface-subtle border border-border-subtle rounded-lg text-ink text-sm"
      />
