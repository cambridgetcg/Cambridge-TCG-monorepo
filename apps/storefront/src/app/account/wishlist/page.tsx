        ...
        <div className="flex items-start justify-between mb-6 flex-wrap gap-3">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="bg-surface-subtle hover:bg-surface-subtle border border-border-subtle text-ink font-medium rounded-lg px-4 py-2 text-sm transition-colors"
            >
              Import CSV
            </button>
            <Link
              href="/account/profile"
              className="bg-ink hover:opacity-90 text-page font-semibold rounded-lg px-4 py-2 text-sm transition-colors"
            >
              + Add from profile
            </Link>
          </div>
          ...
          {(["pending", "matched", "fulfilled", "all"] as const).map((f) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-lg border whitespace-nowrap transition-colors ${
                filter === f
                  ? "bg-accent-wash text-accent-strong border-accent/30 font-medium"
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {filtered.map((item) => {
              ...
              return (
                <div
                  key={item.id}
                  className={`relative bg-surface border rounded-lg overflow-hidden ${
                    av?.matched ? "border-ok/40" : "border-border-subtle"
                  }`}
                >
                  {av?.matched && (
                    <div className="absolute top-0 left-0 right-0 bg-ok text-page text-[10px] font-bold uppercase tracking-wider py-1 text-center z-10">
                  ...
                  <div className="relative aspect-[5/7] bg-surface-subtle">
                    {item.image_url ? (
                      ...
                      <div className="absolute inset-0 flex items-center justify-center text-ink-faint text-xs">No image</div>
                    )}
                  </div>
                      ...
                      <div className="mt-1">
                        <div className="flex items-center gap-1">
                          ...
                          <input
                            ...
                            className="bg-surface-subtle border border-border-subtle rounded text-xs px-2 py-1 w-16 focus:outline-none focus:border-accent"
                            autoFocus
                          />
                          <button
                            onClick={() => saveMaxPrice(item)}
                            disabled={busy === item.id}
                            className="text-[11px] bg-ink hover:opacity-90 text-page font-semibold rounded px-2 py-1"
                          >
                            Save
                          </button>
                          <button
                        ...
                        {item.sku && (() => {
                          ...
                          if (s === undefined) {
                            return (
                              <button
                                ...
                                className="text-[10px] text-accent hover:text-accent-strong underline mt-1"
                              >
                          ...
                          return (
                            <div className="mt-1 text-[10px] text-ink-muted">
                              ...
                              <span className="text-ink-faint"> — {s.explanation}</span>
                            </div>
                          );
                        })()}
                      </div>
                      ...
                      <div className="flex items-center gap-1 mt-1 text-xs">
                        ...
                        {item.sku && (
                          <button
                            ...
                            className="text-ink-faint hover:text-accent text-[10px] ml-1"
                          >
                            edit
                          </button>
                        )}
                        ...
                        <div className="flex justify-between text-ink-muted">
                          ...
                          <span>
                            {av.p2p_price != null
                              ...
                              : <span className="text-ink-faint">no ask</span>}
                          </span>
                        </div>
                        <div className="flex justify-between text-ink-muted">
                          <span>Store</span>
                          <span>
                            {av.store_price != null
                              ...
                              : <span className="text-ink-faint">no stock</span>}
                          </span>
                        </div>
                      </div>
                    )}
                    ...
                    <div className="flex gap-1.5 pt-2">
                      {item.sku && av?.p2p_order_id && av.p2p_price != null && max != null && av.p2p_price <= max && (
                        <Link
                          ...
                          className="flex-1 text-center text-[11px] bg-ink hover:opacity-90 text-page font-semibold rounded px-2 py-1.5 transition-colors"
                        >
                          Buy P2P
                        </Link>
                      )}
                      ...
                      <button
                        ...
                        disabled={busy === item.id}
                        className="flex-1 text-[11px] bg-surface-subtle hover:bg-danger/20 text-ink-muted hover:text-danger rounded px-2 py-1.5 transition-colors disabled:opacity-40"
                      >
                        Remove
                      </button>
                    </div>
