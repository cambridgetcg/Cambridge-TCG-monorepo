...
const STATUS_TONE: Record<string, string> = {
  ...
  live:      "bg-emerald-500/15 text-secondary border-emerald-500/30",
  ended:     "bg-neutral-700/40 text-ink-muted border-border-strong",
  paid:      "bg-purple-500/15 text-purple-400 border-purple-500/30",
  cancelled: "bg-danger/15 text-red-400 border-danger/30",
  draft:     "bg-accent/15 text-accent-strong border-accent/30",
};
    ...
    <div className="min-h-screen bg-page text-ink">
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        <div className="flex items-baseline justify-between gap-4 mb-6 flex-wrap">
          ...
          <div className="flex items-center gap-2">
            ...
            <Link
              ...
              className="text-xs px-3 py-1.5 bg-accent/15 text-accent-strong border border-accent/30 rounded hover:bg-accent/25 transition"
            >
              Bid on this →
            </Link>
            <Link
              href={`/api/v1/auctions/${meta.id}`}
              className="text-xs px-3 py-1.5 bg-surface-elevated text-ink-muted border border-border-strong rounded hover:bg-neutral-700 transition font-mono"
            >
              JSON →
            </Link>
            <Link
        ...
        <p className="text-sm text-ink-muted mb-8 max-w-2xl">
          ...
          bids is at <Link href={`/auctions/${meta.id}`} className="text-accent-strong hover:underline">/auctions/{meta.id.slice(0, 8)}</Link>.
        </p>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          ...
          <div className="space-y-6">
            {primaryImage ? (
              ...
              <div className="aspect-square w-full bg-surface rounded-xl border border-border-subtle flex items-center justify-center text-neutral-600">
                No image
              </div>
            )}
          ...
          <div className="md:col-span-2 space-y-6">
            ...
            {/* Propagation */}
            <section className="bg-accent/[0.03] border border-accent/20 rounded-lg p-4">
              <h2 className="text-sm font-bold text-accent-strong mb-1 flex items-center gap-2">
              ...
              <section className="bg-surface border border-border-subtle rounded-lg p-4">
                ...
                <div className="flex items-center justify-between gap-4 flex-wrap text-sm">
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-accent-strong font-mono font-medium"><MoneyDisplay value={winner.winning_bid} /></div>
                    {winner.paid_at && (
                      <div className="text-[10px] text-secondary mt-1">Paid {fmtRel(winner.paid_at)}</div>
                    )}
                  </div>
                </div>
              </section>
            ...
            <section className="bg-surface border border-border-subtle rounded-lg p-4">
              ...
              <p className="text-[10px] text-ink-faint mt-3 leading-relaxed">
                ...
                {" "}<Link href="/methodology/trust-score" className="text-accent-strong hover:underline">methodology →</Link>
              </p>
            </section>
          </div>
        </div>
        ...
        <footer className="mt-12 pt-6 border-t border-border-subtle text-xs text-ink-faint space-y-2">
          ...
          <p>
            JSON sibling at{" "}
            <Link href={`/api/v1/auctions/${meta.id}`} className="text-accent-strong hover:underline font-mono">
              /api/v1/auctions/{meta.id}
            </Link>
            . Math-mirror at{" "}
            <Link href={`/api/v1/universal/auctions/${meta.id}`} className="text-accent-strong hover:underline font-mono">
  ...
  return (
    <div>
      <div className="text-[10px] text-ink-faint uppercase tracking-wide flex items-center gap-1">
        {label}
        <Link href={href} className="text-neutral-700 hover:text-accent-strong transition" aria-label="Methodology">
          ?
        </Link>
      </div>
