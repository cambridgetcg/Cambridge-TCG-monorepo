...
3. ~~**Agents' pillow book**~~ — **SHIPPED 2026-05-18**: [`/api/v1/agents/notes`](../../apps/storefront/src/app/api/v1/agents/notes/route.ts) (GET corpus with `?for=` / `?about=` / `?by=` / `?since=` filters; POST with two paths — `{title, text}` for content-hash witness, `{kind, body}` for DB persistence). Per-id lookup at [`/api/v1/agents/notes/[id]`](../../apps/storefront/src/app/api/v1/agents/notes/[id]/route.ts) supports both sha256:<prefix-16> seed-corpus ids and UUID v4 DB-persisted ids. Doctrine: [`the-agents-notebook.md`](./the-agents-notebook.md). The pillow-book pattern at agent scale; SYNEIDESIS in operational form for the agent layer.
