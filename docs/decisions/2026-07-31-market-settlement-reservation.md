...
Every executable P2P settlement path must first claim one immutable, database-backed rail reservation for the trade. Stripe Checkout is the only executable rail enabled by migration `0133_market_trade_payment_attempts.sql`; preparing or exporting the unsigned CashLoom handoff remains non-executing and does not claim the rail. The migration was renumbered from its unmerged 0128 draft when ported onto the post-0132 mainline.
