...
2. **Cardmarket — partner approval first.** A historical price-guide download path and dormant proxy proof-of-concept are not evidence of current permission. The source stays blocked/no-fetch; the proof-of-concept is retired. Graduation requires written Cardmarket approval plus a reviewed record of allowed endpoints, access, storage, display, deletion, image and redistribution terms.
