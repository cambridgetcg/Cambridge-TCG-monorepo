...
function canonicalJson(value: unknown): string {
  ...
  throw new TypeError(
    "Answering Rhyme canonical JSON only accepts normalized JSON values.",
  );
}
...
export function canonicalAnsweringRhymeStatement(
  ...
  if (!isNormalizedAnsweringRhymeStatement(statement)) {
    throw new TypeError(
      "Canonicalization requires the normalized value returned by validateAnsweringRhymeStatement() or prepareAnsweringRhymeStatement().",
    );
  }
  return canonicalJson(statement);
}

export function canonicalAnsweringRhymeStatementBytes(
  statement: AnsweringRhymeStatement,
): Uint8Array {
