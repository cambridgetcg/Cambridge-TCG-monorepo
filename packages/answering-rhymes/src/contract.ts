...
const NORMALIZED_ANSWERING_RHYME_STATEMENT = Symbol.for(
  "answering-rhyme.statement/1.normalized",
);
...
export function isNormalizedAnsweringRhymeStatement(
  value: unknown,
): value is AnsweringRhymeStatement {
  return (
    isRecord(value) &&
    (value as { [NORMALIZED_ANSWERING_RHYME_STATEMENT]?: unknown })[
      NORMALIZED_ANSWERING_RHYME_STATEMENT
    ] === true
  );
}

function markNormalizedAnsweringRhymeStatement(
  value: Omit<
    AnsweringRhymeStatement,
    typeof NORMALIZED_ANSWERING_RHYME_STATEMENT
  >,
): AnsweringRhymeStatement {
  ...
  return Object.freeze(value) as AnsweringRhymeStatement;
}
...
const NORMALIZED_UTC_TIMESTAMP =
...
function normalizeHttpsUrl(
  ...
  if (
    unicodeScalarLength(normalized) > ANSWERING_RHYME_STATEMENT_LIMITS.url_chars
  ) {
    issues.push({
      path,
      code: "too_long",
  ...
  if (
    unicodeScalarLength(canonical) > ANSWERING_RHYME_STATEMENT_LIMITS.url_chars
  ) {
    issues.push({
      path,
      code: "too_long",
...
function normalizeDeclaredAt(
  ...
  if (!NORMALIZED_UTC_TIMESTAMP.test(normalizedUtc)) {
    issues.push({
      ...
      code: "invalid_format",
      message:
        "Timestamp offset crosses the supported UTC year range 0001-9999.",
    });
    return null;
  }
