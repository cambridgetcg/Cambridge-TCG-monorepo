...
export async function fetchCardmarketPublicFileWithMeta(
  ...
  // EventBridge/Lambda owns durable retries. A transport-local Retry-After
  // sleep could otherwise outlive the collector's absolute deadline.
  const fetcher = createFetcher(ctx, meta, { max_retries: 0 });
  ...
  try {
    for (let redirects = 0; redirects <= MAX_REDIRECTS; redirects += 1) {
      ...
      if (currentUrl.pathname !== sourceUrl.pathname) {
        void response.body?.cancel().catch(() => undefined);
        throw new CardmarketPublicFileError(
          "redirect-not-allowed",
          "Cardmarket public-file responses may not substitute another artifact identity",
