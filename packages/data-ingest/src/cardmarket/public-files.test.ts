...
describe("cardmarket/public-files fetch", () => {
  ...
  it("rejects a successful response that substitutes another game artifact", async () => {
    const substitutedUrl = PRICE_URL.replace("price_guide_1", "price_guide_6");
    const fetch = vi.fn(async () =>
      responseAt(substitutedUrl, '{"priceGuides":[]}', {
        status: 200,
        headers: { "Content-Type": "application/json" },
      }),
    );
  ...
  it("does not sleep or retry 503 responses inside the durable scheduled attempt", async () => {
    const fetch = vi.fn(async () =>
      responseAt(PRICE_URL, '{"message":"later"}', {
        status: 503,
        headers: {
          "Content-Type": "application/json",
          "Retry-After": "86400",
